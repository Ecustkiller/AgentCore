...
- **自动标题**：侧边栏 UX，非记忆层；不进 Agent 上下文。云/本地均在首条用户消息可用后并行铸题（只用用户首句，`assistant_reply=""`）。云走 `schedule_title_generation` + SSE `title_generated`；本地 sidecar 无云 SSE，桌面首发并行 `POST …/auto-title`，回合回写仅空标题兜底（`_title_inflight` 时跳过）。禁止首轮后再补铸。**铸题失败必落可读短标题（✅）**：铸题是 best-effort，429 / 超时 / 额度用尽都会走 `fallback_title`——取首条消息**首个有内容的行**（剥 markdown 装饰）、超长则断在最后一个从句边界，而非把整段任务卡正文糊进侧栏（线上 `llm.rate_limit_no_retry` → `chat.title_failed` 后的实际观感）。**429 不在这层重试**：退避与放弃归 LLM 网关（`llm.rate_limit_no_retry` 即那层已判定不该重试），标题层再叠一次只会把用户多晾几秒。降级落 `chat.title_degraded`（`reason` 归因），否则侧栏里降级标题与模型产出无从分辨。
...
条目化后（✅）文件页不再有「记忆 / 规则 / 文档」三夹：按全局 / 各文件夹分区列条目，每条露生效徽章与 `description`，并显示常驻用量。条目右键含**「这条不对…」/「恢复使用」**（纠错通道唯一入口，前者先弹确认摊开连带面）；被停用的条目名划掉并标「已停用」，也不再报它的常驻占用（它已不占池）。
