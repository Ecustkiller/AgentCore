...
**纠错通道**：过时记忆必须能退出注入。写侧防幻觉管的是「别记错」，管不了「记对过、现在不对了」。入口：`documents.disputed_at` + 文件页右键「这条不对 / 恢复使用」，且**只有 `ai_maintained` 条目可新标**。手写提示词用删除；工具箱提示词不提供停用（已标的存量仍可「恢复使用」，否则账号层会卡死）。→ 见代码与 `PATCH /v1/documents/{id}` 的 `disputed`

- **只有用户能标，且只能在文件页对 AI 维护的记忆标**。手写条目不是「记错了」而是「不要了」——删。**严禁**扫对话原文猜「用户是不是在否认某条记忆」再自动标记——那是 `intercept-discipline.mdc` 点名否决的意图分类器，且此处误伤代价特别高：误判一次就静默关掉一条用户真正依赖的规则，而用户不会收到任何提示。用户在界面上点，是唯一有明确对象、明确意图、且当场可撤销的输入。
...
- **停用前先摊开连带面**：用户从文件页（或存量对话尾卡）里的**一句话**追过来，这一刀却落在**整个条目**上。右键「这条不对…」先弹确认，按小节列出每一条 bullet 并报条数；措辞只说「停用」，不说「删除」。读不到正文时**不编造条数**。→ `fileWorkbench/DisputeEntryDialog.tsx`
- **句子级：搬走，不是标记**：让用户只否他看见的那一句。主入口是文件页「这条不对」；存量 `kind=semantic` 对话尾卡每行仍可一键搬走（新巩固卡不再发）。不弹确认，toast 带撤销；过后从「最近学到 → 已移走的记忆」放回。→ `memory/dispute_line.py`
  ...
  - **入口必须把改动喊出去**：文件页、存量对话尾卡、找回面「最近学到」各有缓存——在一处否掉一行却不让另一处失效，用户去找就是「什么都没发生」。故行组件的回调（`onMemoryChanged`）是每个宿主的义务，不是可选装饰。
...
- **计量用字符数，取消条数上限**：条目化后 `MAX_INSTRUCTION_DOCS` 失去意义（一条可长可短）。真实成本是 token，但本闸意在防无意膨胀而非精算成本，字符数确定性好且不绑某个模型的 tokenizer。闸的数字不进产品文案。**行尾仍可标每条常驻占用**（`always_chars`）：职责与满池卡片的 `quota_holder` 同源——回答「该删谁」；不足千字（含 0）一律不标。工具箱提示词页用货架卡点开条目，不画常驻构成条（「该删谁」看对话配额卡的 `quota_holder` 与卡上 ≥千字的副标题）。冷启动占位与「已建但还空着」的条目长相一致。文件夹作用域的计量仍是「全局 ∪ 本文件夹」（`global_chars` / `project_chars`，已落库 wire），只给闸与卡片用，文件页不再画两段进度条。
...
**治理靠可见性**：配额停摆仍推 `kind=quota` 卡（**什么没写进来、谁占着池子**）——这是取代「权威分档」的机制，AI 无法自我提权。闲聊收尾、探索、`remember` **不**发 `kind=semantic` 巩固卡；常驻写进了什么，看文件页，不在对话尾等一张「写进去了什么」。存量 `semantic` 行仍可读，与 `quota` 共用同一套行组件（`MemoryUpdateItemRow`），故工具箱提示词「最近学到」对旧卡仍同一读法。情景沉淀只落 `memory_episodes`，**不出卡**。纠错通道入口是文件页「这条不对」，不依赖对话尾巩固卡。
...
**「画像 / 偏好默认常驻」不需要运行时守卫**：生效档是条目自身的属性，而 AI 写入常驻（探索 / 文件页归并）的主要动作是**把新事实归并进已有条目**——**归并不改目标的生效档**。`偏好.md` / `画像.md` / `导航.md` 是常驻，`主题/*.md` 是按需；没有路径能让画像悄悄滑成按需（除非用户自己改）。真正要判生效档的只有**新建**条目那一刻，且判错的代价是「新事实进错了地方」而非「核心画像失效」（旧条目仍在、仍常驻），这个量级交提示词判据即可（几乎每个任务都要用的身份 / 合作方式 → 常驻；某领域厚知识 → 按需），符合 `intercept-discipline` 阶梯 1，不上闸。否决表「偏好/画像改 on_demand」仍成立。
...
⏳ **余项**：取消 `role` 三分；探索按 `description` 归位（现状探索仍整文件重写文件夹画像；**不得预置空条目**）。用户技能入基座 ✅（系统 Skill 真源留代码；官方 HOW 只读，不占槽 → [工具与能力 · 用户技能](/docs/03-AI核心/工具与能力系统.md#用户技能与可编目录)）。
...
4. **跨文件夹名册**（派生，**非记忆**）：**不进** CEO 常驻提示。当前桌路径 + id 只在 `<工作区>` 工作台行；其它桌用 `list_folders`（回执含完整 `rel_path` + id）/ `resolve_folder`（嵌套同名须完整路径，歧义不静默猜）。换桌 HOW 在 `delegate` `target_folder_id`。不进 `<设定>`，不得挤掉 always 记忆。
...
> 闲聊收尾**不写常驻**。情景摘要落 `memory_episodes`、立即标已消化、**不注入**、不出卡。常驻（`偏好.md` / `画像.md` / `导航.md`）只由 `remember`、探索幕、文件页写入。✅ **主题不由闲聊写**。⏳ 探索仍是整文件或按节合并，目标形态按 `description` 归位；**不得预置空条目**。

**常驻检验**：一条属于常驻 ⟺ 以后**几乎每个任务**都要用它来选怎么做（全局），或这张桌上几乎每个任务都要用（文件夹画像 / 导航）。「跨话题仍可能有用」不够。一次性查询、本场任务结果、本机路径不进常驻。
...
- 导航写入判据：一条有用 ⟺ 下次能省掉一个动作；闲聊/纯偏好场导航零变化；探索幕是导航写入者（含旁路刷新）。
...
| 画像.md | 我是谁 / 这桌是什么 | 用户陈述；几乎每个任务（或这张桌上每个任务）都要用；文件夹层另加探索摸仓 | 探索幕 · 文件页 |
| 导航.md | 下次能省掉的一步 | 动作清单实证 | 探索幕首建；旁路刷新增量 |
| 主题/*.md | 以后还会来查的厚知识 | 不能从产品或工作区当场拿到 | 探索幕（含旁路刷新）· 文件页。`remember` 仍只写用户规则 |
...
### 冷启动探索幕（勿与已撤销的「巩固冷启动」混名）

闲聊巩固不再写常驻，故无「空核就降抽取门槛」。唯一还叫冷启动的是探索幕。
...
| 工具箱提示词「停用」当第三档加载 | 生效只有常驻 / 按需；官方只读、我的删除；纠错不是加载开关 |
...
| 闲聊摘要自动写入常驻（偏好/画像/导航） | 一场任务渣进每回合上下文；常驻只留几乎每个任务都有用的，入口是 remember / 探索 / 文件页 |
...
| 扫用户原文猜「是不是在问功能」再跳过沉淀 | 意图分类器；正确滤网是闲聊不写常驻（入口 remember / 探索 / 文件页）、主题不在闲聊出口，不是给对话分类 |
| 条目正文指向盘上路径（挂牌用户仓库 md） | 第一版不做：模型本就能 `file_read`，增量收益仅一行 `description`；疼了再加 |

查看/编辑：对话内 `remember`（增改删列）与工具箱提示词（「最近学到」 `/toolbox/mine/skills?updates=1`）+ 各文件夹 ``.agentcore``、右坞工作区同一 ``.agentcore``（常驻槽位摊开；主题默认折在「主题」夹）+ CAS 双轨；semantic diff 可搬层纠错。→ 见代码：`fileWorkbench/AgentCoreSection.tsx` · `EntriesSection.tsx` · `workspace/WorkspacePanel.tsx`

**核心叶可清空、槽位保留**：`画像.md` / `偏好.md` / `导航.md` 的**正文**可经文件页右键「清空」去掉（危险确认；走已有 `PUT /v1/users/me/memory/files/{kind}` 空正文，与编辑器抹空同一条）。空了就不注入。列表仍占位（与冷启动占位同一套）。`DELETE /v1/documents/{id}` 仍拒核心叶——固定文件名是加载协议，不是第三条内容类型。探索 / 文件页 / `remember`（规则）之后仍可能再写入；拦复活是另步。
...
条目化后文件页不再有「记忆 / 规则 / 文档」三夹：账号共用提示词在工具箱（「最近学到」 `/toolbox/mine/skills?updates=1`）；各文件夹条目与过程稿合在 ``.agentcore`` 里（默认折叠；打开后根上是常驻、`主题/*.md` 收进默认折叠的「主题」夹（按需），行上不标常驻/按需徽章，仍可带 `description`，稿夹摊平为 `工作稿`/`research`/`debate`/`reviews`）。「新建条目」在 ``.agentcore`` 标题行右侧（折叠时也能建）。AI 维护的条目右键含**「这条不对…」**（先弹确认摊开连带面）；已停用的（含手写存量）划掉、标「已停用」、可「恢复使用」，也不再报它的常驻占用（它已不占池）。手写条目不要了就删。核心叶右键是**「清空」**（不是「删除」）。
