...
- **`conditional` 直接删**：它自诞生即 DB 预留值、API 一直拒写（`api/routes/documents.py` 自述），**存量零行**——「生效两档」不是数据迁移，只是把死值从 CHECK 摘掉。
...
**`description` 怎么来**：**异步**生成、**只在空时**生成、非空永不自动覆盖。异步是因为同步会把一次纯数据写入变成依赖 LLM 可用性——模型挂了条目就存不进去。「仅空时生成」则零新字段地绕开「这条摘要是 AI 拟的还是用户手写的」：否则要么不敢更新、要么覆盖用户手写。代价是 AI 拟的摘要可能随正文过时，用户要刷新就**清空**（清空即重生成），这是个可发现也好解释的出口。空 `description` **不是错误状态**——目录里只显示名字，不报错不阻塞。AI 巩固自己写的条目时本就在模型上下文里，顺手带上摘要，不走这条补写路径。
...
**基座边界**：进基座 = 会被注入的条目（纯 DB 正文）。不进 = 运行产物（`research` / `debate` / `reviews`）、代码、附件、用户仓库自带 md → 盘上文件 + `file_read`。**系统 Skill 正文也不进基座**（真源在代码）但**参与 `<按需目录>`**——「不进基座」讲的是存储归属，不是可见性。情景摘要与 `_memory_meta.json` 降级为巩固管线内部状态、移出基座。`文档/` 退化成纯产物目录。

**配额：闸在写侧，读侧全量**：取消固定文件名（`偏好.md` / `画像.md` / `导航.md`）后常驻集合失去天然上限。定案是**常驻满了就不许再往常驻加，读侧永远全量注入、不截断**——让「挤」发生在用户有上下文的时刻（他正在写这条），而不是在某个回合里悄悄决定他哪条规则今天不生效。引擎不替用户挤：无分池、无自动淘汰、无 AI 溢出决策。

- **读侧整套预算逻辑删除**：`compose_injected_rules` 的贪心装填、`_keep_rank`、`RuleFragment.authority`、超预算静默丢弃全部移除；常驻块退化成「全部常驻条目按显示顺序拼接」。权威档取消后 `keep_rank` 第二维本就失去依据——不是给它找新排序，是不再需要排序（无人被淘汰）。
...
| 2 | 条目化：frontmatter 为真源、取消类型、生效两档、配额闸移到写侧 | DB 内**一次性**迁移（列值写进正文头部）；读侧为净删除 |
...
> 现状 ✅（按需侧「单目录 + 单工具」已随步 1 落地）。目标形态（常驻拼成单块、写侧配额闸）见开头「目标形态」节 ⏳。
