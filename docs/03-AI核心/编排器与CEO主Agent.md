...
要不要 `delegate` 看信息结构，不看场面名。收益只认两条：墙钟（弱依赖切片并行）和注意力（容量、不许见面、进 CEO 窗后面每轮都在）。人数、工种头衔、「看起来像团队」不是理由。
...
| 4 | 谁的窗？CEO 窗跟会话走，偏派；工人 / 嵌套 lead 的窗跟这次 run 走，切不出去的切片留下 | 根派 vs 嵌套豁免 |

「这一回合装得下」不是路由键。真正贵的是过程进 CEO 窗之后每轮都还在（档位取舍：大文件进 CEO 窗此后每回合都付）。仓库里一行配置还不在窗里 → 根侧仍派；同一刀已在工人窗里 → 嵌套不拆。正文已在窗里的一行新建 → 自己写。
...
嵌套委派：硬上限 `depth≤3`（合法链 CEO → depth1 → depth2 → depth3 叶子；depth&lt;3 默认获 `delegate`，`replan` 在已有子计划后挂上），无 `can_delegate` 字段。worker 工具集缺省全量（内部装配）；CEO **不必也不应**手填 `tasks[].tools` 收窄（填了也不生效）。depth=1 captain 对路径 B 形 brief（成果级目标·约束·验收、本轮无结构钉成单切片）→ **优先**先再 `delegate` 补编制再整合（优先级 nudge，非硬流程、非「未嵌套禁写」）；豁免：单文件 / 已钉薄壳 / 切不出去的现场 / 小修·机械单步。整里程碑 M0 / 空仓多模块骨架不在豁免内。**禁止**「凡大活必嵌套」；失败只回退提示词，不升级闸。根 CEO 编制 HOW → `consult(staffing)`；嵌套 lead → `consult(lead_subteam)`（队员专属、持 `delegate` 才进目录与 fetch）。两把 `delegate` 的 description 分叉：根侧协调立即返回 / 嵌套阻塞等到子队收工。
...
| 「实质讨论」当委派触发器 | **否决**（切不出去的对比仍直答；能切开或过程不该进 CEO 窗才派） |
