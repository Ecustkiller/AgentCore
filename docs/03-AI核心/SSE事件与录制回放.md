...
- **`turn_queued`**（✅ EPHEMERAL）：同对话 FIFO 排队确认（`queue_id` / `position` / `queue_depth`；已进入 accepting 窗口、又赶不上下一步而在收口升队时带 `degraded_from: "steer"`。从未进窗的是普通 FIFO，不带该字段。协调插话不带）。协调插话升 FIFO 时亦向 live sink 发射（可进条、可取消）。三条入队路（经典 FIFO / 协调升队 / steer 收口回落）均经**对话信号道**广播给跟播该会话的各端，一条连接只收一份；内容权威仍是 `GET …/queued-turns`，本帧只作「变了」信号。
- **`turn_queue_started`**（✅ EPHEMERAL）：FIFO 出队开跑，新回合 sink 首帧（`pop_next` 后、`stream_chat` 前，先于 `message_start`）。正文在帧上：`content` 必填；可选 `user_message_id` / `attachments` / `agent_mentions`。已经拿到该帧的连接用它提前插入同一用户行并清该 `queue_id`。入场权威是这一次开跑复用已落库用户行时，段首 `message_start` 上的同名点名；cursor 回放头从本 sink 的这一帧拷贝。裸 `message_start` 不得清条。**否决**靠裸帧猜出队。**否决**为此新增事件。队列能否跨进程重启见运行时三模型 · 队列耐久。reload 靠 REST。`turn_queued` / `turn_queue_cancelled` 仍只是「变了」信号。
...
- **`desk_provision_wait`**（✅ EPHEMERAL）：云桌开通短等（绑定当前 server root 时 `ensure_workspace_desk`）；`waiting` 进出。桌面空气泡「正在准备云端环境」——禁空 Thinking… 冒充开机。缺桌仍不装配 `run`。→ 见代码：`tools/sandbox/desk_provision.py` · `runtime/events/run.py:desk_provision_wait`
...
- **`run_failed.failure_kind`**（✅ additive）：协作图失败脸优先按此类贴文案——`model`→「模型中断」、`call`→「调用失败」；缺省→「失败」/空 error「调用失败」。旧 journal 若仍带其它 kind，未知值丢弃、不当专名脸。禁前端扫正文猜脸。→ 见代码：`RunFailureKind` · `runtime/events/payloads/run.py`
- **`tool_use_end.status=redirect`（✅ additive）**：选错工具通道（短执行里 dump/grep 源码、`web_fetch`↔`read` 互斥、长驻走错短核等）运行时拒执行并改道。项目级 install/test/typecheck **不是**改道——统一 `run` 按命令分类进验证核直接跑。模型面 `result` 仍是失败回执；过程步不是 `error`。`failure.code` 驱动折叠标题（「改用搜索」）；`failure.message` 可缺——自愈下一步不进用户面。旧 journal 的 `status=error` + 改道 `failure.code`（含已停发的 `project_verify_redirect`）由 fold 归一为 `redirect`。→ 见代码：`runtime/engine/tool_channel_redirect.py` · `runtime/engine/tool_failure_face.py`
...
- **旧 journal / 录制回放**：不在现行 `EventType` 的名字 skip；跨回合同图走 `run_plan.prev_execution_id`。推进卡不在 live resolve union，不再投影。
...
2. **「谁能看这个会话」的授权模型**——✅ 文件夹成员可打开桌上对话（可编辑续跑/跟播/审批/停止；只读只看）；外人仍 404、不泄漏存在性。权威 → [工作区 · §八、协作桌（文件夹成员）](/docs/02-架构/工作区.md)。跨进程总线与订阅配额仍是扩容（本表 1、3）。
