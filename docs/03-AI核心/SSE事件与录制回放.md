...
> **权威**：SSE 事件协议、契约生成、多端跟播、录制/回放。前端消费 → [前端技术 §十二](/docs/04-前端/前端技术与架构.md)。
>
...
- **接入姿势**（✅）：重放段 → `: attach-caught-up` 边界 → 实时段。客户端据此把首段整段缓冲后一次折，否则已完成的 worker 会再演一遍 running→completed。
- **增量重放**（✅）：`Last-Event-ID` 决定**发什么、不决定读什么**——服务端照旧读整回合 journal，因为四处判定（是否结构化回合 / 已覆盖的 run 集 / `agent_id` 回填 / worker 全文拼接）必须扫过全表才成立，改成 `seq > 游标` 查会让它们翻面成「worker 正文整段重发或静默丢失」。贵的是网络与客户端重折，不是那一次主键索引扫描，故**过滤发生在产出事件之后**。→ 见代码 `runtime/events/attach_replay.py`

**两条指令都由服务端下达，客户端不拿屏上状态猜**——没有它们的年代，桌面 follow 重连分支猜错把正文折了两遍：

- **`full_replay`**（段级，落在段首 `message_start`）：本段是整回合重放，先重置该 `message_id` 的正文 / 思考 / process 时间线，再折本段。增量段与 live 首帧都不带 = 同回合重开、往后接。两条重放路径同令：走 journal 的合成段首，走 sink 内存历史的把段首那帧改写成带标记的副本（有内容却没有 `message_start` 就补一帧）。
...
2. **「谁能看这个会话」的授权模型**——现状严格 owner-only（非本人 404 且不泄漏存在性）；是否复用[共享空间](/docs/02-架构/双模式工作区.md)成员三档待定。
