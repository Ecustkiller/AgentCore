...
2. **组团 · delegate / debate**：活有规模 / 有结构时（实质交付物，或成规模广度调查），CEO 在 ReAct 循环里调 `delegate` 下达一批子任务；`depends_on` 边定形 DAG（无依赖即并行），是「串行 / 并行 / 混合」的唯一开关。对抗性多视角则调 `debate`。fan-out / 主持人循环前经编排层**开工卡**（`team_preview`，`primitive` 判别；能力授权合并进卡）向用户征询：delegate 授权并开工（可带嘱咐）/ 停止；debate 授权开赛（可带开赛嘱咐）/ 停止；用户全局**自治三档**（`always_ask` / `first_grant` / `full_auto`）决定问的频度——`full_auto` 不弹卡。→ [编排器 §一·delegate / §五](/docs/03-AI核心/编排器与CEO主Agent.md) · [安全权限与治理 §三](/docs/05-平台与运维/安全权限与治理.md)
3. **调度 · WaveScheduler**：`build_run_plan` 一次建全 DAG，`WaveScheduler` **连续依赖调度**——节点自身依赖一就绪、并发有空位（`max_parallel=8`，worker = asyncio 协程）即发射，**不等同一拓扑波的其余节点**；只在决策边界（CHECKPOINT / BIND / SCOPE，即「波边界」）让出、把控制权交回 CEO。→ [执行引擎 §一·受监督的波循环](/docs/03-AI核心/执行引擎架构设计.md)
...
| 原语 · `delegate` | CEO 下达子任务 | `depends_on` 定 DAG；≥2 worker 根侧默认协调（立即返回、同回合再调追加同一张图）；`coordinate=false`/单 worker/`finalize`/嵌套 lead/checkpoint 闸开走阻塞 | [编排器 §一–二](/docs/03-AI核心/编排器与CEO主Agent.md) |
...
| 带现场续派 / `RunSession` 续写 | [多轮编排与同人续派](/docs/03-AI核心/多轮编排与同人续派.md) | Run 模型 → [执行引擎 §八·Run](/docs/03-AI核心/执行引擎架构设计.md) |
...
| 恋综场景（第二场景） | `simulation/scenarios/show/` + `simulation/show/` | 6 人卡司、`heart_pick` 赛制状态机、期编排器、导播编译器（run → EpisodeManifest）、节目 API；离线生产：`python -m agentcore.simulation.show produce --episode 3`；产品定案 → [AI 恋综场景提案](/docs/06-规划/AI恋综场景提案.md) |
| 观测客户端 | `apps/town/`（Unity 6 LTS） | AgentTown：Session / REST / SSE / Offline Demo / 3D；HUD 顶栏（含 fps-label）·底栏·左右轨；恋综节目模式（左轨「节目」/ `?episode=3`，离线消费 EpisodeManifest）→ [AgentTown 客户端](/docs/04-前端/AgentTown客户端.md) |
...
| 流式 stall 闸 | 流式读空闲超 `engine_llm_stream_idle_timeout_seconds` | **未提交**（尚无 content/tool_call；reasoning 不算提交）→ 透明整请求重试（次数对齐 provider `_MAX_RETRIES`，另受 idle×2 总预算约束），耗尽后抬成 `LLMTimeoutError`；**已提交** → `aborted` salvage 半成品 | `runtime/engine/stream.py` |
