...
| 原语 · `delegate` | CEO 下达子任务 | `depends_on` 定 DAG；根侧默认协调（**含单 worker**；立即返回、同回合再调追加同一张图）；阻塞仅嵌套 lead / 画布人工把关 | [编排器 · `delegate` / `replan`](/docs/03-AI核心/编排器与CEO主Agent.md#delegate--replan) |
...
| `ask_user` 检查点 | [检查点与开工卡](/docs/03-AI核心/检查点与开工卡.md) | 挂起机制 → [运行时三模型 · Interaction](/docs/03-AI核心/运行时三模型与挂起.md)；编排字段 → [编排器](/docs/03-AI核心/编排器与CEO主Agent.md) |
