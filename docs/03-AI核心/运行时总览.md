...
2. **组团 · delegate / debate**：活有规模 / 有结构时（实质交付物，或成规模广度调查），CEO 在 ReAct 循环里调 `delegate` 下达一批子任务；`depends_on` 边定形 DAG（无依赖即并行），是「串行 / 并行 / 混合」的唯一开关。对抗性多视角则调 `debate`。fan-out / 主持人循环前经编排层**开工卡**（`team_preview`，`primitive` 判别；能力授权合并进卡）向用户征询：delegate 授权并开工（可带嘱咐）/ 取消；debate 授权开赛（可带开赛嘱咐）/ 取消；会话 **PermissionAxes** 的 `team_kickoff` 轴（`always` / `rules` / `skip`）决定问的频度——`skip` 不弹卡。→ [编排器 §一·delegate / §五](/docs/03-AI核心/编排器与CEO主Agent.md) · [安全权限与治理 §三](/docs/05-平台与运维/安全权限与治理.md)
...
| 角色 · worker | 默认可再委派（depth&lt;3）、隔离上下文 | 持全套工具动手；depth&lt;3 默认获 `delegate`+`replan`；不能反问（改用 `escalate`）；一次性但可恢复 | [协作模式 §一](/docs/03-AI核心/Agent协作模式.md) |
| 原语 · `delegate` | CEO 下达子任务 | `depends_on` 定 DAG；根侧默认协调（**含单 worker**；立即返回、同回合再调追加同一张图）；`coordinate=false`/`finalize`/嵌套 lead/checkpoint 闸开走阻塞 | [编排器 §一–二](/docs/03-AI核心/编排器与CEO主Agent.md) |
...
| 原语 · `ask_user` | CEO 唯一发问 | 通用澄清（可阻塞挂起）；`blocking` 正交 | [检查点与开工卡 · §一](/docs/03-AI核心/检查点与开工卡.md) |
...
| 系统提示词装配 / 工作区概览 / 缓存 / 长对话压缩 | [执行引擎 §三·长对话压缩](/docs/03-AI核心/执行引擎架构设计.md) · [执行引擎 §七](/docs/03-AI核心/执行引擎架构设计.md) | 装配 `runtime/context`（`PromptContributor` 插件 + `SectionOrder` 声明序，稳定前缀护缓存）；记忆注入 → [记忆系统 §二](/docs/03-AI核心/Agent记忆与知识系统.md)；是否统一 + 扳机（向量 RAG 仍缓；扳机 A 已吃满为按需三合一 ✅、扳机 B 已落地为写侧常驻配额闸 ✅ `memory/always_quota.py`）→ [上下文工程](/docs/03-AI核心/上下文工程.md)；工具输出 head+tail 截断 → 见代码 `core/text.truncate_head_tail` |
| ReAct、收敛、`finish_guard` | [执行引擎 §四](/docs/03-AI核心/执行引擎架构设计.md) | — |
| SSE 事件契约 | [SSE 事件与录制回放](/docs/03-AI核心/SSE事件与录制回放.md) | → 见代码 `runtime/events/` |
| Turn / Journal / Interaction 三模型 | [运行时三模型与挂起](/docs/03-AI核心/运行时三模型与挂起.md) | — |
...
记忆系统 §二（注入）→ 执行引擎 §七（装配）/ §三（长对话压缩）→ 上下文工程（是否统一 + 两个扳机）
...
| 流式 stall 闸 | 流式读空闲超 `engine_llm_stream_idle_timeout_seconds` | **未提交**（尚无 content/tool_call；reasoning 不算提交）→ 透明整请求重试（次数对齐 provider `_MAX_RETRIES`，另受 idle 窗口数=`_STALL_BUDGET_IDLE_MULTIPLIER` 约束；生产 multiplier=2 ⇒ 首窗后仍可再试一次，不以 `remaining < idle` 边沿误砍），耗尽后抬成 `LLMTimeoutError`（worker/DAG 走现有 `run.failed` → partial_failure）；**已提交** → `aborted` salvage 半成品 | `runtime/engine/stream.py` |
