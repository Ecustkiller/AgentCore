...
2. **组团 · delegate / debate**：CEO 持已装配的全套干活工具。要不要派看[信息判据](/docs/03-AI核心/编排器与CEO主Agent.md#信息判据)，不看场面名；闲聊、窗口里已有的短答与小落盘、纯启服自己回。加人只换墙钟或注意力，人数不是目标。`depends_on` 边定形 DAG（无依赖即并行），是「串行 / 并行 / 混合」的唯一开关。用户点名开辩才调 `debate`（日常多视角走组队）。组队 / 点名后的开辩**直接开跑**。会话 **PermissionAxes** 管写盘 / 执行 / 本机 Host，不再含组队确认轴。→ [编排器 · `delegate` / `replan`](/docs/03-AI核心/编排器与CEO主Agent.md#delegate--replan) · [安全权限与治理 §二](/docs/05-平台与运维/安全权限与治理.md)
...
5. **监督 · replan 续跑**：队员报 `escalate kind=scope` 时，调度器在波边界让出，CEO 调 `replan`（steers / add / stop）续跑**同一张 DAG**。手写下游未定 → 先跑再 add。→ [编排器 · `delegate` / `replan`](/docs/03-AI核心/编排器与CEO主Agent.md#delegate--replan)
6. **升级 · 向用户**：worker 撞上死结用 `escalate`（`blocking=false` 问而不停 / `blocking=true` 挂起求决策）经 CEO 中转到用户；CEO 自己在关键岔路用 `ask_user` 通用澄清。→ [协作模式 §二·escalate](/docs/03-AI核心/Agent协作模式.md) · [检查点 · §一](/docs/03-AI核心/检查点.md)
...
| 系统提示词装配 / 工作区概览 / 缓存 / 长对话压缩 | [执行引擎 · 三、执行管线 · 长对话压缩](/docs/03-AI核心/执行引擎架构设计.md#长对话压缩) · [执行引擎 · 同窗 window compact](/docs/03-AI核心/执行引擎架构设计.md#同窗-window-compact当轮唯一瘦身) · [执行引擎 · 七、提示词架构](/docs/03-AI核心/执行引擎架构设计.md#七提示词架构) | 装配 `runtime/context`（冻 tools + 冻 system + 只追加）；记忆注入 → [记忆系统 §二](/docs/03-AI核心/Agent记忆与知识系统.md)；`consult` 只拉 HOW → [上下文工程](/docs/03-AI核心/上下文工程.md)；**提示词写什么** → [上下文工程 · 提示词设计原则](/docs/03-AI核心/上下文工程.md#提示词设计原则)；头衔咒语 / 身份≠资深 → [角色提示与身份](/docs/03-AI核心/角色提示与身份.md)；工具输出 head+tail 截断 → 见代码 `core/text.truncate_head_tail` |
...
记忆系统 §二（注入）→ 执行引擎 · 七、提示词架构 / 三、执行管线（长对话压缩 · 工人 window compact · 压缩文件账）→ 上下文工程（Assembler / 按需与写侧配额 / **提示词设计原则**）
