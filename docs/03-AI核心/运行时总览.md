...
4. **协作 · 产物中转**：Agent 间**不直连**——上游产物经调度器注入下游（`pass_through` 共享预算水填充 + 首尾保留 / 落盘者递指针）；另有三条被动通道（扇出兄弟感知 / 拓扑位置 / `escalate`）。→ [协作模式 §二](/docs/03-AI核心/Agent协作模式.md)
...
| 角色 · worker | 默认叶子、隔离上下文 | 持全套工具动手；不能反问（改用 `escalate`）；一次性但可恢复 | [协作模式 §一](/docs/03-AI核心/Agent协作模式.md) |
| 原语 · `delegate` | CEO 下达子任务 | `depends_on` 定 DAG、`model_preference`、`result_handling`、`can_delegate`(≤2 层) | [编排器 §一–二](/docs/03-AI核心/编排器与CEO主Agent.md) |
...
| 原语 · `escalate` | worker 唯一向上通道 | 阻塞 / 非阻塞；`kind=scope` 喂波边界纠偏 | [协作模式 §二](/docs/03-AI核心/Agent协作模式.md) |
...
| Agent 间通信、`escalate` | [Agent 协作模式 §二](/docs/03-AI核心/Agent协作模式.md) | 禁止 worker↔worker 直连 |
| 上下文 8 通道、`run_context` | [上下文传递可视化](/docs/03-AI核心/上下文传递可视化.md) | 记忆注入 → [记忆系统](/docs/03-AI核心/Agent记忆与知识系统.md) |
| 系统提示词装配 / 工作区概览 / 缓存 / 长对话压缩 | [记忆系统 §5.5–5.6](/docs/03-AI核心/Agent记忆与知识系统.md) | 装配 `runtime/context`（`PromptContributor` 插件 + `SectionOrder` 声明序，稳定前缀护缓存）；压缩 → [执行引擎 §十三](/docs/03-AI核心/执行引擎架构设计.md)；是否统一 + 扳机（向量 RAG / consult_rule / 预算 Assembler 均已缓）→ [上下文注入统一性讨论](/docs/07-规划/上下文注入统一性讨论.md)；工具输出 head+tail 截断 → 见代码 `core/text.truncate_head_tail` |
...
记忆系统 §5（上下文模型）→ 执行引擎 §十三（长对话压缩）→ 上下文注入统一性讨论（是否统一 + 两个扳机）
