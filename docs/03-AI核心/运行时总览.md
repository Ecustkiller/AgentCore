...
| 系统提示词装配 / 工作区概览 / 缓存 / 长对话压缩 | [执行引擎 §三·长对话压缩](/docs/03-AI核心/执行引擎架构设计.md) · [执行引擎 §七](/docs/03-AI核心/执行引擎架构设计.md) | 装配 `runtime/context`（`PromptContributor` 插件 + `SectionOrder` 声明序，稳定前缀护缓存）；记忆注入 → [记忆系统 §二](/docs/03-AI核心/Agent记忆与知识系统.md)；是否统一 + 扳机（向量 RAG 仍缓；扳机 A 已吃满为按需三合一 ✅、扳机 B 已触发为写侧配额闸 ⏳）→ [上下文工程](/docs/03-AI核心/上下文工程.md)；工具输出 head+tail 截断 → 见代码 `core/text.truncate_head_tail` |
