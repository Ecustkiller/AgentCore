...
| 权威 | 角色 |
|---|---|
| `drive*` + `CoordinationSession`（含 `live_plan` 恢复写入） | **写**：派发、跑批、协调态执行真相 |
| `pipeline_view` | **只读**进度投影（注入 CEO），禁止回写执行态 |
| `isomorphic` / thrash | drive **入闸**（拒同构再派），≠ 前端图折叠 |
| 桌面/手机协作图 | SSE 消费后的 **读投影**（`projectExecution` 等） |

路径写权账本（§三）与「图上怎么画」正交：账本管能不能写文件；图只反映进度。跨文档边界总述 → [编排器 · 执行写路径 vs 进度读视图](/docs/03-AI核心/编排器与CEO主Agent.md#执行写路径-vs-进度读视图)。**不做**本轮把 drive 与协作图事件流硬合成。
