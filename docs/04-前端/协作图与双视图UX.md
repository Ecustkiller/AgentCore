...
**按人干预挂在节点上（定案 ✅）**：「只改这个人的方向 / 只停这个人」两枚按钮悬在卡片下沿之外（`AgentNodeInterveneBar`，不盖 face 的「他正在干什么」），**跟随注意力**——悬停 / 该卡是右坞钉住的那位 / 改方向草稿开着才现身，其余时间整条不渲染（零噪音；透明隐藏会把隐形按钮留在 Tab 序里）。曾埋在「展开图 → 点节点 → 开右坞 → 找入口 → 展开文本框」之后，于是图上按人显示每个队员在干什么、用户能操作的却只有整轮停止。**不可用不隐藏**：队员跑完确实改不动也停不了，但按钮留在原位变灰 + 给一句为什么（判定与文案两端共用 `protocol-fold-kit/runIntervene`，桌面右坞 run 详情与手机队员详情同源）；**否决**「不可用即 `null`」——挖到该在的位置却发现按钮不见了，用户会以为自己找错了地方，扑空一次就再也不试。辩论幕不开放改方向属**能力本就不存在**，该按钮整体不渲染（见 `planCapabilities`）。
