...
**状态条只报战绩（定案）**：细工具栏——生命周期图标（无解说句）+ `n/m` + 用时 + 花费 + 回放/折叠 + 「在画布打开」。**否决**任务摘要 / 「等待你确认」等解说标题、「团队完成」大字、条上「停止整轮」（主槽输入框已有停止）、合成草稿行寄居状态条（成员细节走图节点）。**否决**「接续 N 次」（对比只走画布顶栏「对比」；图上「续 ×N」只陈述）、「互相把关」计数行（升级卡/节点已是事实；气泡脚也不挂）、「同时开工省下」并行省时（反事实对比；图已表达并行）、已拍板开工卡摘要「预计 N 人开工」（开工卡只在等拍板；开跑后图即编制。resolved 预览不寄居状态条，图已出现时也不回时间线废卡——与「N 个 Agent」已删同因：和图重复）。
