...
移动端图简化 ✅：竖排 TeamView 有意简化（非待做画布；→ 见代码 `TeamView` · parity `InlineTeamGraph`）。跨会话多任务总览、无障碍、流式字级动画 ⏳。**断线只读（N4-A）**：可浏览已缓存对话与本机文件（只读），不能发送 / 改文件 / 跑 AI；「本地引擎」= 本机执行更快，**不是**离线模式（推理仍要云）。完全离线（本机推理 + 本机消息库）仍 ⏳。消息收藏 ✅：命令面板「已收藏」facet；**否决**侧栏独立列表。
