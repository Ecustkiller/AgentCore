...
- **上·工作区分组**（按文件夹可折叠）：组头显云/本地图标·名称，hover「⋯」/ 右键 = 查看全部对话 / 浏览文件 / **归档全部对话**（批量归档该项目下活跃对话，非 `Folder.archived`）/ 删除项目（与文件页共用 `DeleteProjectDialog`）；组内复用 `ConversationItem` 列 Top 5，超出走「更多」跳 `/conversations` 并聚焦该组。按近活跃排序、组数 ≤6（溢出走「查看全部对话」）；展开态按 `folderId` 持久化（`useSidebarStore`，显式切换优先；无记录默认折叠、唯含当前对话的组自动展开）。
...
- **交锋叙事线**（过程，三层渐进披露 · 渲染为**竖向时间线**）：顶一条 **收敛信号带**（`ConvergenceBand`：每轮一个轴点、按「交锋信号」`roundSignal` 着色——在飞脉动 / 收敛绿 / 有交锋蓝 / 各说各话灰，让用户 30 秒看懂辩了几轮、第几轮收敛，对标 Kialo minimap）；轴上每轮三层 → **L1**（一句话焦点 clamp 成轴标题 + 小结 + 裁判徽章，串成认知推进线）→ **L2**（主持人小结 + 裁判理由：真交锋 / 有无新论据 / 是否收敛）→ **L3**（论点级全文 + 交锋边：各方发言全文懒展开，靠 `run_id` 关联辩手节点取全文、按身份色描边、点角色下钻右侧详情；裁判逐轮抽取的**交锋边「谁驳谁」**（`from→to` + 要点，载于 `clashes`）渲染为**可点列表**——点一条即展开本轮发言、把对应两方发言格加品牌色环高亮 + 滚动定位，把「引用连线」落成可直达发言的导航）。焦点回归「一句话」：主持人 prompt 收紧到 ≤30 字短语（像小标题），前端再 `truncate` 兜底。
- **顺序自适应**：决策类（正反/红队）决策简报（裁决 hero：倾向 + 置信信号条 + **置信成立条件全文**）先行、叙事线紧随；探讨类（多方圆桌）把 **观点光谱**提到叙事**之前**的英雄区（`RoundtableSpectrum`：各视角核心主张按身份色 + 综合观察，glanceable「先给地图再读论点」），叙事线居中、简报小结（共同焦点 / 分歧 / 待解）收尾。

三形态（正反辩论 / 红队挑刺 / 多方圆桌）共用同一**时间线骨架**（轴点 + 连线 + L1/L2/L3 + 可点交锋边），但**逐轮研判（L2）按形态差异化**：正反 = **逐轮记分卡**（对阵行 + 交锋 / 论据 / 收敛三维判定，取自裁判真实字段、不编造比分）；红队 = **风险看板**（本轮判定「发现风险 / 已挖尽」+ 方案方 + 把交锋边改标「风险点」）；圆桌 = **通用研判**（中性裁判徽章，形态差异主要由叙事前的观点光谱英雄区承载）。参与方泛化为「立场标签」（`sides[*].name`，不再硬编码正/反）。

**呈现位置 · 归画布（✅）**：辩论双产物**不内联聊天**——它是「过程」，按「一份数据两种渲染」（§六）落在**画布放大态的「交锋叙事」标签页**（与「协作图」切换、**默认交锋**、图作次级；回放本回合则默认落图、让时间线在图上播放）。决策理由：辩论的 DAG 是 CEO→主持人→辩手 固定低信息形状，不值得与叙事争半屏；聊天的线性阅读流也不该被过程全文塞满（结论自有 CEO 收尾文字承载）。入口=协作图内嵌状态条的「在画布打开」（聊天侧只留状态条的「辩论」pill 作信号、不再有产物卡）。

**live→收场 无跳跃（✅）**：交锋页的 `DebateBody` 单实例贯穿进行中→收场，不再「进行中一棵树、收场另一棵树」。`toDebateModel` 把进行中（transport-only `debateRounds` + 辩手 run 树）与收场（权威 `debate_result`）归一成同一个 `DebateModel`——同一实例不卸载，简报与辩题在收场处**淡入**，叙事是一条贯穿的统一轮次列表。

- **能统一的根因**：后端**故意**把进行中逐轮（`DebateNarrativeRound`）与收场逐轮（`DebateRoundInfo`）设计成**同构孪生**（仅 `verdict` 可空之差，见 [`辩论编排设计.md §7.4`](/docs/03-AI核心/辩论编排设计.md)）；旧两套树是「实现冒充数据模型」的产物。
- **不闪的键**：逐轮单元按 `round_no` 复用、逐轮发言格按辩手 `run_id` 复用——后端两路同 id（`{moderator}_r{k}_{side.key}`），同一辩手发言跨 live→收场 是同一 React 实例、不重挂。
- **顺序去跳跃**：本场会话**亲眼看着**收场（外壳曾渲染进行中）时，简报一律追加在叙事**下方**、不顶走正读的轮次；只有重载（一上来即收场态）才按上「顺序自适应」把决策类简报摆到叙事之上。
- **被替代**：旧「收场 `DebateProducts` + 进行中按 `group` 两栏并排兜底」两套实现——收场一瞬整卡重挂、入场动画重播、卡头变脸、叙事折叠跳跃，已删。

→ 见代码：桌面 `components/chat/debate/`（`model.ts` 归一器 + `roundSignal` 信号派生 + `Narrative.tsx` 时间线叙事 + `ConvergenceBand.tsx` 收敛信号带 + `Brief.tsx` 简报卡 / 圆桌 `RoundtableSpectrum` 光谱英雄区）+ `DebateCompare.tsx` 的 `DebateBody`（双产物面，挂于画布放大态 `components/graph/CanvasZoomedTurn.tsx` 的「交锋叙事」页）、手机 `apps/mobile/src/components/DebateView.tsx`（精简双产物，⏳ 桌面时间线/信号带重设计尚未镜像到手机）。
...
| 双产物 | **不内联聊天**——双产物（决策简报 + 交锋叙事线，§4.1）归画布放大态「交锋叙事」页，入口=状态条「在画布打开」；进行中逐轮叠加焦点/小结/发言、收场原位补简报与辩题，无 live→收场 跳跃——`DebateBody`（在 `CanvasZoomedTurn`，`debate/model.ts` 归一 live+收场） |
...
**工作区删除 vs 对话整理（✅ 已确定 · 对标 ChatGPT/Codex 分层）**：**对话层**——归档（可恢复，隐藏活跃列表）/ 永久删除（用户视角不可恢复），见 §一；**工作区（Folder）层**——**不做「归档项目」**（行业亦无独立概念），侧栏降噪靠**组头「归档全部对话」**（批量归档该项目下活跃对话）或在 `/conversations` 按文件夹批量归档。**删除项目**（`/files` 工作区根右键 + 侧栏组头「删除项目…」· 现有 `folders` soft-delete，共用 `DeleteProjectDialog`）：仅删容器——其下对话 `folder_id` 清空并落入「未分组」、**不**删对话记录；项目文件归 Folder 所有，保留至保留期（默认 30 天，见 [`双模式工作区.md` §七](/docs/02-架构/双模式工作区.md)）后 sweeper 物理清理。**否决** ChatGPT 式「删 Project 级联删全部聊天+文件」——真实工作区（本地盘 + OSS）下对话是索引、文件是资产，级联过狠。**否决** `Folder.archived` 第三整理层。**删除确认**（✅ 已落地）：Dialog 展示「N 条对话将移入未分组」+ 项目文件 30 天保留期 + 引导用归档整理，并可选勾「同时归档其下全部对话」（默认勾选，勾选后文案改为「将一并归档」）。**「彻底删除项目」**（✅ 核按钮、另入口）：一次性清对话+文件+快照（`DELETE /v1/folders/{id}/permanent`），侧栏组头 + 文件页工作区根右键各提供入口、须输入项目名确认；本地项目不删用户磁盘文件。→ 见代码 `components/folders/PermanentDeleteProjectDialog.tsx`、`agentcore/folders/permanent_delete.py`。
...
>
> （→ 见代码 `stores/sidePanel.ts`、`components/layout/SidePanel.tsx`、`components/chat/detail/RunDetailBody.tsx`、`components/workspace/WorkspacePanel.tsx`（`WorkspaceMode`）、[`前端技术与架构.md` §9.2 / §9.4](/docs/04-前端/前端技术与架构.md)）本节为关键决策。
