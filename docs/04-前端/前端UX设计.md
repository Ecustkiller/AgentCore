...
- **来源卡可信度徽标 ✅**：`citations` 契约可选 `tier`（向前兼容；缺字段=legacy 不渲染）。桌面 `SourceCards` / 手机 `Citations` 展示「官方 / 媒体 / 待评 / 弱源」徽标，配色走语义 token（官方=`success`、媒体=`primary`/`accent`、待评=`muted`、弱源=`warning`）。分级单源在后端 `citation_tier_for_url`；**显式引用 `weak` 时弱源徽标必显**；硬拦域名永不挂卡。→ 见 [工具与能力系统 · 引用质量闸](/docs/03-AI核心/工具与能力系统.md)。

- **回合证据台账通道与 `#rN` 溯源 ✅**：独立 SSE `evidence_ledger`（delta 增量 / entries 全量 / `cited_ids`）经双端 fold 进 `ProjectedTurn.evidenceLedger` / `citedIds`；`ProjectedCitation.id` 与 REST Citation 对齐（缺字段=legacy）。正文 `#rN` 经 remark 解析；桌面来源卡 / 角标可按 id 打开溯源面板（query · 是否深读 · tier · 登记者等——条目模型与辩论 `#eN` 徽章同源，入口可双）。历史消息：**有 id 走 id、无 id 走池序 `[n]`** 双轨，禁止假定全库已迁移。**P2 主卡 = `citations_event` 仅成稿引用集**（未引用命中只在台账/痕迹面）；辩论场级台账仍走辩论事件，不占本通道。主张须证 → [引用即出处提案 · P3](/docs/06-规划/引用即出处提案.md)。
...
- **回合内联错误卡 ✅**：直播走纯传输 `error` SSE；**重载**从 `turn_end.error` 投影回放同一张卡——软失败（CEO `raise_on_error=False` 收口）也由 settle 落 `turn_end.error` + `usage.error_code`（原先仅 salvage 路径落，软失败刷新后只剩空气泡）；缺错误载荷的历史行由前端按「空正文 + `finish_reason=error`」合成兜底卡。卡内动作按错误类型分流：鉴权/余额类 →「去设置」、连通性类 →「重试」，同类连通性失败会话内第 2 次起升级为配置引导文案；**「复制诊断信息」**一键复制 `conversation_id` / `trace_id` / `message_id`（有则写入）供用户报障；空正文失败轮在下一轮 prompt 折叠为失败注记（见 [执行引擎 §历史重建](/docs/03-AI核心/执行引擎架构设计.md)）。`code` 走 `lib/errors.ts` 单点翻译。
...
**交付状态卡（掐断透明化 ✅）**：`delivery_status` 在 partial / blocked 时出 `DeliveryStatusCard`（正文下、产物卡上；`delivered` 不出卡、由产物卡承载）。缺口行可选 `reason` 徽标——已知 `token_budget`（预算触顶）/ `worker_timeout`（运行超时）/ `degraded_handoff`（降级交接）；未知 reason 忽略（向前兼容）。手机 TeamView 交付区块同口径。→ 见代码 `DeliveryStatusCard.tsx`、`TeamView.tsx`；后端 [`编排器 §交付状态`](/docs/03-AI核心/编排器与CEO主Agent.md)。
...
| 深层诊断指标 | 诊断模式骨架 + 调度 `BatchMetrics` 已落地（§十）；worker LLM 窗口已并入「收到的上下文」诊断态 ✅；收敛 `turn_metrics` 仍 ⏳ |
