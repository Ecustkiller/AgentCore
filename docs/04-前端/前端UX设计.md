...
- **底部**：「已归档 (n)」（有归档才显，跳 `/conversations` 聚焦已归档视图）+「查看全部对话」入口。
...
- **执行中**（`RunningStrip`）：转圈 + 任务摘要 + 进度 `completed/total` + 进度条；尾部控件（停止 / 折叠图 / 在画布打开）。Agent 状态/工具/输出在下方图节点上呈现。
...
>
> ✅ **前端重构已落地（2026-06 拍板 → 落地）**：交锋叙事前端从「直播=对话式 `LiveChat` + 复盘=擂台 `Arena`/时间线 `Narrative` + 终局=裁决 hero，按 `model.settled` 分相位切换」**收敛为一间「统一辩论室（IM 群聊）」**——live 与收场是同一条流的「未完成 / 已完成」，无 phase 切换、无独立复盘视图。主视图 `DebateStream`，擂台（`DebateArena`）降为放大态唯一可选透镜（论证地图 `ArgMap` 因「视图过多」一并退场）；旧 `LiveChat`/`Narrative`/`ConvergenceBand`/`UserTake`/`DebateCompare`/`ArgMap` 已退场。行为契约见 §4.1，组件去向见 §4.1b。

### 4.1 统一辩论室：IM 群聊（✅ 已落地）

把整场辩论收敛成**单条群聊时间线**——直播与收场是同一条流的「未完成 / 已完成」（`DebateStream` 单组件 + `toDebateModel` 归一，沿用现成、不重挂）：把原本只服务直播的 IM 群聊范式**扩成全生命周期 + 全形态的唯一主视图**，其余降为可选透镜。

**四条已确认决策（用户拍板）**：

| 维度 | 决策 | 要点 |
|---|---|---|
| **布局** | 观察者群聊（单列）：辩手全靠左（身份色头像分阵营）、用户靠右（第三方追问）、主持人=居中系统消息 | 一套覆盖 正反/红队/圆桌；对抗感靠**阵营色 + 引用回复 + 置顶阵营条**，不靠左右分栏（行业实践：群聊「他人一侧·你一侧」，versus 双栏仅 1:1 / 头对头比较成立，后者归「对比擂台」透镜） |
...
| **落点** | 画布放大态「辩论室」（群聊=新主视图），入口=状态条「在画布打开」 | 复用 `CanvasZoomedTurn` 视图切换器；聊天保持干净 |

**行为契约（IM 流）**：

| 概念 | IM 呈现 |
|---|---|
| 辩手发言 | 成员气泡（身份色头像）；直播流式「正在输入…」+ 光标 → 收场定格 markdown 全文 |
| 轮次 | 居中分割线「第 N 轮 · 焦点」 |
| 主持人小结 / 裁判 | 居中系统消息（有交锋 / 各说各话 / 已收敛） |
...
| 决策简报 / 裁决 | 流末「主持人终审」= 唯一结论面（完整简报体 `BriefCard`：倾向+置信 / 争点分诊：价值之争 warning / 渐进披露依据 / 圆桌光谱 / 「裁决过程」钻取 / 终局站队软对照）；顶部仅议题头 +「结论 ↓」锚 |
...
**旧范式（已退场）**：直播 `LiveChat` 群聊气泡、复盘 `Arena` 擂台左右对开（2方）/ `Narrative` 时间线（多方）、终局裁决 hero（`GavelCard`/`YourVerdictHero` 等）浮顶 + 可展开复盘，`DebateCompare.tsx` 按 `model.settled` 分相位。问题：多范式（直播 vs 复盘、2方 vs 多方）+ phase 切换 + 左右语义仅 2 方成立 → 用户反馈「乱」。已由 §4.1 统一辩论室替代。

**组件去向**：~10 套范式 → **1 主视图（结论归位流末）+ 1 可选透镜**（✅ 已落地）。

| 现组件 | 去向 |
|---|---|
| `LiveChat` | ✅ 删 → 升为唯一主视图 `DebateStream`（全生命周期 + 全形态） |
| `Arena`（擂台） | ✅ 改 → 可选「对比擂台」透镜 `DebateArena`（仅正反 2 方，= Chatbot Arena Battle 用法；内部擂台网格复用） |
...
净效：主视图 `DebateStream`（单流·结论归位流末 `FinalVerdict`）+ 1 可选透镜（对比擂台）。放大态视图切换器：`[群聊]`（辩论默认）`[对比擂台]`（仅正反 2 方）`[协作图]`（`[并行时间线]` 当有并行数据）。

→ 见代码：`components/chat/debate/`（`model.ts`/`Brief.tsx`/`Continue.tsx`/`Interjections.tsx` 复用；`DebateStream.tsx` 主视图、`Arena.tsx` 导出 `DebateArena` 透镜；已删 `LiveChat`/`Narrative`/`ConvergenceBand`/`UserTake`/`DebateCompare`/`ArgMap`）+ 放大态接线 `graph/CanvasZoomedTurn.tsx`（视图切换器）+ 概览节点 `graph/FocusedTurnNode.tsx`（脚抽屉只剩「版本对比」，辩论无 peek、走放大态）+ 手机 `apps/mobile/src/components/DebateView.tsx`（暂不镜像，待跟进）。
...
| 辩论全程 | **不内联聊天**——辩论全程（逐轮发言 + 决策简报 + 交锋，§4.1）归画布放大态「统一辩论室（群聊）」主视图，入口=状态条「在画布打开」；live 与收场是同一条流的「未完成/已完成」、无跳跃——`DebateStream`（在 `CanvasZoomedTurn`，`debate/model.ts` 归一 live+收场） |

### 4.3 老板介入、追问与续辩（✅）

辩论中途介入**复用** [`辩论编排设计.md §六`](/docs/03-AI核心/辩论编排设计.md) 的 opt-in 逐轮交互（进程内挂起 `DEBATE_ROUND` + 流内轮边界行动条 `SteeringBar`，§4.1b 已由旧 `DebateRoundDecisionCard` 归流至此）：
...
> ✅ 已落地（§4.1）：功能不变、前端落点已归流——三选一 + 追问 = 直播态「轮边界行动条 + composer」（`DebateStream` 内 `SteeringBar`）；复盘「你的追问·是否被承接」= 流内右侧消息原位（不再单列 `Interjections`）。

### 4.4 站队（✅ · 会话内态）

用户侧轻量标注，**绝不改 AI 裁决内容**（守中立：站队只对比）。store `debateUserTake.ts`（按 `turnId` 分桶）+ `DebateStream` 内的 `StanceVote`（落点见 §4.1）：

- **站队** `StanceVote`（发言气泡上的投票 chip，直播+终局）：点「站这方」记倾向（身份色高亮、仅你可见·不影响 AI 裁决）；终局在流末「主持人终审」`FinalVerdict` 附「你 vs AI」软对照（`leaning` 文本含该方名→看似一致，只提示不下硬判）。
...
曾把辩论读成节点（各方 + 最强主张）+ 有向边（各轮 `clashes` 谁驳谁）的 `ArgMap` 次级透镜，**已随「视图过多」收口移除**（2026-06）：它最 niche，且与「协作图」同为节点图、形式重叠（用户易感「又一个图」）。攻防结构的逐轮交锋仍可在「群聊」的引用回复 + 「对比擂台」的本轮交锋点读到。放大态辩论视图现为 **群聊（默认）+ 对比擂台（仅正反 2 方）** 两个，见 §4.1b / §6.5。
...
- **聚焦节点固定高度**（`FOCUS_NODE_HEIGHT`）：脚抽屉（仅「版本对比」；读答案 / 重读提问已改走右坞「内容 tab」，辩论回合走放大态「统一辩论室」无 peek）与内嵌图**共享这块固定高度**（开抽屉图区缩、抽屉占下半承版本列 `DRAWER_H`、图区相应再缩），节点总高恒定 → 下方回合堆叠偏移不被挤动。**否决**抽屉撑高节点（触发动态高度 → 重算堆叠）。
...
**✅ 收口**：图上指挥与比对卡片已全数上画布——`BackgroundTaskCard`（云端 / 后台任务卡片，非阻塞 · 跨对话的另一类）入指挥台（见 §6.2）；`RevisionCompare`「版本对比」由聚焦回合表头 chip 唤出、就地落在脚抽屉（逐字复用聊天同款卡的 `bare` 形态，逐版本仍下钻右坞 run 详情）。至此聊天侧的指挥 / 比对卡片在画布均有归处（**定向唤回**「修订 vN」本身仍 CEO 驱动、无用户触发入口，其结果另作 `AgentNode` 节点画在聚焦回合 DAG 上）。

→ 见代码：`stores/ui.ts`（`conversationViews` 持久化、只落画布 override）、`pages/ConversationPage.tsx`（视图切换 + 偏好读取）、`chat/StatusStrip.tsx`（团队回合「在画布打开」入口）、`graph/ConversationCanvas.tsx`（持久累积 + LOD + 发布画布 active/聚焦回合 + 后台任务同步驱动）、`stores/commandPanel.ts`（画布→指挥台区桥：active/聚焦 + 折叠态）、`graph/CanvasDecisionPanel.tsx`（`useCommandRegion` 派生 + 自动浮出 + `CommandRegion` 折叠区，复用 §三 同款决策卡片）、`layout/SidePanel.tsx`（顶部承载指挥台区，§十）、`graph/TurnSummaryNode.tsx` / `graph/SimpleTurnNode.tsx` / `graph/FocusedTurnNode.tsx`（内嵌 `GraphView` + 端点开右坞内容 tab + 版本对比脚抽屉〔辩论走放大态辩论室〕 + 提示牌）、`graph/CanvasCommandBar.tsx`（常驻命令栏 + 后台云端派发）、`chat/RevisionCompare.tsx`（`bare` 形态嵌画布脚抽屉）、`lib/agentIdentity.ts`（身份延续）。

### 6.5 放大态视图：群聊（辩论室）/ 对比擂台 / 协作图 / 并行时间线 ✅ 已落地

放大态（`CanvasZoomedTurn`）顶栏给一个**视图切换器**（≥2 个可用视图才出现），同一回合在多种渲染间切换：**群聊**（统一辩论室 `DebateStream`，辩论主视图、仅辩论回合，§4.1）/ **对比擂台**（`DebateArena`，正反 2 方左右对开复盘，仅 2 方辩论，§4.1b）/ **协作图**（依赖结构，恒在）/ **并行时间线**（时间真相，仅当本回合有并行执行数据）。辩论默认落群聊、其余默认落图、回放本回合落图让时间线在图上播放；**并行时间线恒为可选透镜、从不作默认**。
...
**对话落点表达（✅ 草稿期工作区选择器 · B3+）**：新对话草稿输入框工具行挂 `DraftWorkspacePicker`。**默认不选**时不展示「自动」chip，仅轻量「归入项目…」；不选 ≡ 桌面 local-first 懒建（零门槛不变）。选了项目或「仅云端」后显示确认 chip（项目名 / 云端 + 清除）。面板标题「对话归属」；首项「不归入项目」；项目列表经搜索选取（空闲时展示最近 3 条，替代 Top6 平铺）；桌面另含「选择本地文件夹」与「仅云端（随手聊）」。**B4**：草稿期 `@` / 浏览附加来自某项目的文件时，内联提示是否归入该项目（`DraftWorkspaceAssignPrompt`）。落点经 `pendingNewChat*` 传给首发建会话；侧栏工作区组头另提供「新建对话」预填落点。首发后锁定，改由 `WorkspaceModeBar` 承担。→ 见代码 `components/chat/DraftWorkspacePicker.tsx`、`components/chat/DraftWorkspaceAssignPrompt.tsx`、`components/sidebar/WorkspaceGroupHeader.tsx`。
