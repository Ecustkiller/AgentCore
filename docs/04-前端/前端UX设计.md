...
侧栏 + 页面自布局；对话页单栏聊天。内嵌图「在画布打开」→ `TurnDetailPage`；点节点 → SidePanel run tab。输入框：空草稿居中（`card` 摊开配置），首条后 FLIP 落底（`bar`：＋收纳配置 · 输入 · 语音 · 发送）。→ `ChatView.tsx`、`useComposerDockFlip.ts`、`ComposerPlusMenu.tsx`。

**侧栏 IA（方案 C）**：顶=全局置顶（裸聊 + 文件夹内的对话均可；按最近活动；零重复）；中=文件夹分组（Top 5/组、≤6 组；组内仅未置顶）；底=未置顶裸聊扁平（有分组 10 / 无分组 15）。**否决**跨区「最近」、裸聊「未分组」空组、置顶与组内/裸聊双显。三区无标题，分隔线区分。对话**归属中途不改挂**（不支持事后把对话移入另一个文件夹；要换心智→新开）。**≠**手不能达他项——同对话可跨文件夹派工。归档可撤销；删除对用户永久（无回收站 UI）。「对话」导航 = 新建草稿（`/` 唯一真相）。**草稿不可用右坞**（不出现、快捷键/`openPanel` 等也不能打开；有会话后用户再开）。
...
**已否决（聊天）**：Slash、Agent/Team 选择器、产物 Pill、草稿「存储⊥文件夹」双入口。
...
**文件夹=工作区**（✅ **双通道观察期**：云协作推荐 + 本机传统可选）；入口= SidePanel + `/files`。**否决**无推荐徽标的云/本地两平级分段。草稿「在哪工作」单一 chip（✅ 上：入口动作一行——「快速对话 / 新建文件夹 / 从本机导入 / 从 Git 克隆 / 打开本机文件夹」（可标「上次」），**动作名只留动词短语**、落点不占独立行（三项自带来源词，「新建文件夹」靠紧跟「快速对话」+ 与「打开本机文件夹」间的 gap 暗示云端；组标签方案试过已撤），其下弱链「了解区别」开对照说明弹窗；动作区与文件夹列表仍只一条分隔；动作行无通道教学副标题；下：已有文件夹仍示位置，嵌套层带祖先面包屑）。文案与用词禁令权威 → [工作区 §5.1 · §7.1](/docs/02-架构/双模式工作区.md)。默认云；可记上次通道。手机无本机传统。合回 Diff 与下载 ZIP 并排出口（云桌）→ [工作区 §五·§7.6](/docs/02-架构/双模式工作区.md)。删文件夹：对话归档、文件保留期清理；**否决**级联删聊天、`Folder.archived`。**最近删除（✅）**：保留期内可恢复——主路径是删除后的「撤销」toast（多数误删几秒内发现），兜底是 `/conversations` 左栏「视图」组与「已归档」并排的「最近删除」。恢复**故意不含**白板的文件夹归属（`Board.folder_id` 置 NULL 是「白板永不丢失」的设计，原值无处可查）与裸聊 auto-desk 指针（恢复它会造幽灵工作区，下回合自动重建）；本机文件夹那一支的 OS 目录删除与恢复都不碰。前端删文件夹**禁止**逐条走归档接口——那会把成员对话标成「用户主动归档」，恢复时就分不出该解档哪些（只做客户端侧效：卸运行时 / 导航离开 / 刷缓存）。**否决**回收站内「彻底删除」：`permanent_delete` 是四步非原子流程，与恢复交错会留下空壳文件夹；要立刻清盘仍走删除弹窗的勾选框。恢复只认 access session（不给 folders 窄票、不给 AI）；过保留期返 409 如实提示，不对账不重试。「清空本对话产物」仅云端 `conv:` scratch。审批：信任档下文件改动免逐次卡（永久删仍问）。预览矩阵：md 默认阅读；常见图内联（缩放/灯箱，≤10 MiB）；PDF 面板 iframe（≤15 MiB）；HTML 面板显源码，完整效果走统一浏览器壳（→ [前端技术 §9.12](/docs/04-前端/前端技术与架构.md)）；Office/其它二进制不内联（下载或本机默认程序）。`/files` 搜索框筛工作区名 + 已展开树内路径/文件名（非内容搜）。云文件 CRUD 含 copy（`POST …/copy`，与 move 同口径）；删进软删区/回收站，确认文案须诚实可还原。产物主清单认 `delivery_status.artifacts`；呈现口径 → [执行引擎 · 可用性诚实性](/docs/03-AI核心/执行引擎架构设计.md)。「查看改动」与右坞「改动」tab 同源（见 §十）。约定文档徽章复用文件树；**否决**阶段 Tab。协作时间线=读时聚合；**否决**项目级 execution 实体。共享空间并入「项目」段角标「共享」。→ [工作区](/docs/02-架构/双模式工作区.md)；产物卡 → `lib/fileArtifacts.ts`。
...
**主入口不在这个页面**——是对话完成态状态条上的「存为工作流」，画布退居事后微调；辩论回合有意不出这个入口。复跑走 mechanism-direct、**CEO 不在场**，所以拓扑是锁死的：用户在等人关卡处能改内容、改不了拆法。「换个主题再跑」由槽位承担（默认值 = 原轮原值，第一次点「跑一次」时按需抽）。理由与边界 → [工作流固化与复跑](/docs/02-架构/工作流固化与复跑.md)。→ `WorkflowsPage` / `WorkflowEditorPage`。

## 十三、模型与自主度

设置拆「模型」（组合）/「服务商」（BYOK）/「Git 凭据」（G3 云私仓 PAT；Local 说明继承 OS/`gh auth`）。会话选**组合**非裸模型。组合槽位候选来自统一目录（BYOK：default ∪ 厂商预设 ∪ 发现）；另支持对 BYOK 服务商**手填** model id（平台行仍只 allowlist）。设置页对协作/工具调用能力作静态诚实提示（**否决**质量档/自动降级）。**否决**账号级角色→模型矩阵、质量档、组合入口双 picker。**Per-worker 模型覆盖 ✅**：默认跟 Worker 槽；CEO/`tasks[]` 节点显式仍有效；开工卡确认面**不**提供人改模；完成后协作图 peek / 用量详情见 `run.model`——定案 → [编排器](/docs/03-AI核心/编排器与CEO主Agent.md#per-worker-模型覆盖abc-同一功能)。自主度配方三选一（谨慎 / 少打断（默认）/ 托管）→ [安全权限与治理 §三](/docs/05-平台与运维/安全权限与治理.md)。账户默认：桌面经对话权限徽章「设为新会话默认」写入；手机可经设置或会话内「＋」菜单同入口写入。会话内可改四轴（含本机）：**桌面**空草稿 / 画布 `card` 靠底栏权限徽章；有消息后的底栏 `bar` 与**手机**一样收进 composer「＋」。**Composer 通道（✅）**：用户选 **云协作**（快速对话 / 新建文件夹 / 从本机导入 / 从 Git 克隆，推荐）或 **本机传统**（打开本机文件夹 = 本机权威 + 本地引擎；「云协作 / 本机传统」是内部口径词，界面不出现）；选项须写清差异（云=多端同一份、合回才落本机；本机=直改打开的目录、≠ 离线）。默认云；可记上次。**否决**并排「本地引擎 / 云端过桥」实现词；**否决**本机与云平级无推荐。过桥不当新入口。→ [工作区 · §五·§七](/docs/02-架构/双模式工作区.md)。「＋」常显工作区 · Git? · 模型 · 权限 · 附件（合回/ZIP 主入口在 ModeControl → [§7.6](/docs/02-架构/双模式工作区.md)；Composer「＋」不再常驻；遗留 handoff 在 ModeControl 高级/遗留）。顶栏不再展示配方后缀。非法组合不可选；自定义四轴不可存为账户默认。
