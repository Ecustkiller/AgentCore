# AgentTown 客户端规格（Unreal Engine 独立应用 · 路线 B）

> **状态**：🗂️ 提案（2026-07-08 Unity 定案；**2026-07-08 修订为 UE 5.5**）
> **决策**：观测层从 Desktop 内嵌 R3F **迁至 Unreal Engine 独立客户端**；**后端 `simulation/`、REST/SSE 契约、Postgres 落库不变**（非路线 C 全栈分叉）。
> **背景**：[AI 小镇 MVP 开发计划](AI小镇MVP开发计划.md) 原 R3F FE 路线接缝 bug 成本高；产品方定案画面为长期卖点，避免日后二次迁移引擎。
...
| 维度 | 定案 |
|------|------|
| 产品名（工作名） | **AgentTown**（UE 打包可执行文件，如 `AgentTown.exe`） |
...
```mermaid
flowchart TB
  subgraph town [apps/town Unreal Engine 5.5]
    UI[UMG 观测面板]
    World[3D 场景 + NPC]
...
2. **UE 只消费契约**：REST + SSE；不内嵌 LLM、不本地推算 tick。
3. **单一客户端状态机**：`SimulationSession` 为唯一会话真相（禁止 React 式多 store）。
4. **Live / Replay 共用 `ApplySnapshot`**：差异仅在插值 vs 瞬移、是否处理 SSE 增量。
5. **坐标变换单点**：Wire 坐标（Three.js 右手系 Y-up）→ UE Z-up 左手系，仅在 `ApplySnapshot` 内变换（§6.2）。
...
│   └── town/                        # Unreal Engine 5.5
│       ├── AgentTown.uproject
│       ├── Source/AgentTown/        # C++ SimulationSession、REST、坐标
│       ├── Content/                 # 场景、蓝图、资产引用
│       └── Config/
├── packages/
...
| R3F `simulation/town/*` | UE Phase 1 验收后删除；`?preview=1` 暂留至 UE 离线模式就绪 |

**R3F 代码定位**：冻结为**对照实现**，不再新增功能；新需求只进 UE。
...
1. 对每个 agent：`wirePos` → `uePos = (wire.x, -wire.z, wire.y)`（§6.2）
2. 合并 `manifest.personas` 的 `big_five`；bio 见 §6.4
3. Replay：`AActor::SetActorLocation`；Live：`AIController` + NavMesh 设目标
4. 纯视觉偏移 `spawnOffset`（§6.5）加在 UE 侧，**不改** wire 坐标
5. 更新 UI、`tickCache`
...
| Run 列表 | GET | `/runs` | **中期后端增项**（BE-UE-01）；MVP 用本地历史 |
...
与 `agentcore.simulation.types.SimTickSnapshot` 一致（含 `tick_memories`、`governance`、`active_events`、`modifiers`、`metrics`）。C++ struct + `FJsonObjectConverter` 手写对齐 OpenAPI。
...
| UE 场景 | 左手系 **Z-up**；**单点变换**：`ue = (wire.x, -wire.z, wire.y)` → `FVector(X, Y, Z)` |

**验收**：`市场` wire `(24,0,0)` → UE 世界坐标 `(24, 0, 0)`。须加 Automation 单测读 `simulation-region-positions.json`（`Content/Fixtures/` 同步副本）。

> `contract-types` 注释改为「Wire 世界坐标（客户端自行映射至引擎坐标系）」。

### 6.3 区域锚点

单一源：`simulation-region-positions.json` → UE `Content/Fixtures/`（构建或 Editor 同步自 protocol-conformance）。
...
同区域多 NPC 防堆叠：UE 侧 `spawnOffset` 表（对齐 Desktop `TOWN_SPAWN_OFFSET`），**仅影响渲染**，不参与后端位置回写。
...
| 项 | 现状 | 处理 |
|----|------|------|
| `sim.tick_started.agent_count` | fixture 有，TS 类型无 | UE/C++ 容忍可选字段 |
| `SimTickEndedPayload.metrics` | Python 有，TS 无 | UE 读可选 `metrics` |
...
| 层 | 职责 |
|----|------|
| GameMode / Bootstrap | 配置、认证、`SimulationSession` |
| TownMap | 地形、Kenney 建筑、NavMesh、区域锚点 |
| NpcLayer | `Character` + `AIController` + NavMesh；`agent_id` 与后端一致 |
| CameraRig | 鸟瞰 + 跟踪（Phase 2） |
| UiLayer | UMG 面板 |

**NPC 渲染（定案）**：单骨骼 FBX/GLB（`Xbot`）**实例化** + 材质/标色区分居民（对齐现 Desktop，非每人独立 Mixamo 文件）。

**资产（定案）**：`packages/town-assets/` 为单一源；自 `apps/desktop/public/simulation/assets/` 迁出或 CI 同步脚本，避免双份手工维护。Kenney / Quaternius（CC0）+ Fab 免费包可选用。

**性能**：10 NPC ≥ 30 FPS（Windows 中端 GPU）。

**引擎版本**：**Unreal Engine 5.5**（团队统一；5.4 可兼容，以 `.uproject` 为准）。
...
| 阶段 | 策略 |
|------|------|
| MVP | 本地 JSON / `GConfig` 保存最近 12 个 run（对齐 Desktop `runHistory.ts` 语义）；支持 `--run-id` 恢复 |
| 中期 | 后端 **BE-UE-01**：`GET /v1/simulation/runs` 列表（用户维度） |
...
## 11. Desktop R3F → UE 功能迁移对照

| Desktop 组件 | UE Phase | 说明 |
|-------------|----------|------|
...
| Conformance | C++ Automation 消费 `simulation-m1-tick.json` + region fixture |
| UE | Phase 0：`WireCoordinateTransform` Automation；Phase 1：PIE + 打包 smoke |
| 渲染 | `shoot-simulation-town*.mjs` 退役后由 UE 截图测试替代 |
...
- [ ] `apps/town` UE 5.5 + Bearer 调通 create / tick / GET ticks/1
- [ ] 坐标变换 + 1 NPC 到 `市场`
- [ ] Desktop 并行：`session.json` 写入（最小实现）
- [ ] **Go/No-Go**：REST + 坐标 + ≥30 FPS；未过则回评估工期，不硬扛
...
| 7 | 资产 `packages/town-assets`（CC0 Kenney/Quaternius + Fab 免费包） |
| 8 | **Unreal Engine 5.5**；Windows 先行的 macOS 后 |
| 9 | Replay 仅 GET 逐帧；Live 忽略 `tick_frame` |
| 10 | MVP FE 任务映射为 **UE-***（见 MVP 计划 §2.1） |
| 11 | **2026-07-08**：由 Unity 改 UE，避免日后二次迁移；工期 +2~3 周计入第 8 周缓冲 |

---

## 相关文档

- [AI 小镇 MVP 开发计划](AI小镇MVP开发计划.md)（§2.1 UE 任务映射）
