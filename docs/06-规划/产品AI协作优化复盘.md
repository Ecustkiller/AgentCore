...
>
> **治理**：本目录仅 `06-规划`；🗂️ = 讨论中、未承诺落地。决策通过、开始落地后，对应条目迁入 `03-AI核心` 等正式文档，本条可退役或改为索引。
>
...
3. **推进 Phase 2a**：中间可见性 Step 1（redirect 交互）已落地；下一步是 Step 2 scheduler 单人取消 + Step 3 冷重跑（详见 [`多轮编排与队员热修.md`](../03-AI核心/多轮编排与队员热修.md) §十）。
