...
> **产品定位（2026-07-07 定案）**：MVP 卖点 = **「好看、能看的 3D AI 小镇」**——3D 观看体验是核心价值、非可选装饰。故**「基础一档美术（并入 FE-02，用 Kenney 现成低模）+ 30 FPS 硬底线」为必做**；高质量精修（FE-18）/ 进一步性能优化（FE-19）仍可降级；`UT-00`（3D 能否跑通）升级为**卖点命门**，"降级 2D" 不再是优雅退路（见第 6 节）。
...
> **开发模式**：**单人开发者 + 全程 AI 辅助**（无并行团队）。下文「人天」为**工作量估算单位**、非日历天；里程碑对单人基本**串行**（每里程碑内先后端、后其 Unity 前端）。
> **工作量与排期**：committed 工作量 **~108 人天** + ~13 人天降级池（可砍）。**日历排期为假设**——单人串行 + AI 提速，提速倍数无真实数据、待 M0/M1 校准，不承诺硬交付周数（见 §3）。
...
| LLM 网关 | `llm/profiles.py` `llm/resolve.py` | 🟡 | 有 `agent.fast/agent.strong` profile 骨架，但**用户可见的分层选模已永久移除**（原 `api/routes/model_modes.py` 路由已删）；分层解析需参照 `evals/eval_modes.py` 自建 |
...
## 1. 分阶段开发计划（5 里程碑 M0–M4）

### M0：技术 + 产品验证（Spike）— 先行，产出喂入 M1
...
### M4：观测体验与 MVP 交付（核心 ~16.5 人天 + 降级池 ~13 人天）

**目标**：核心 UX 模式齐备（观察 + 跟踪 + 时间轴回放），20+ tick 可回放，3 次同配置运行宏观指标**可测量并如实报告**。资产精修 / 日报 / 性能深调 / conformance 向量列为**降级池**（进度紧张或验收硬需要时才做）。
...
| UT-00 | Unity | **Unity Spike（替代 SPIKE-01）** | `apps/town`：REST Bearer 调通 create/tick/GET snapshot；1 NPC；wire→Unity 坐标变换；Go/No-Go 到 `市场 (24,0,0)`。产出：可运行构建 + FPS 粗测。 | 无 | 2d | SPIKE-02, SPIKE-03 |
...
**M0 小计**：约 **9 人天**（Spike 先行；各 Spike 产出直接喂入 M1，非纯叠加）
...
| FE-14 | 前端(Unity) | 观察模式总览（→ UT-14） | 鸟瞰热力图：区域情绪色块、人口密度；宏观指标折线图（Unity UI 图表控件）。`GET /metrics`。 | BE-25 | 3d | FE-16 |
...
| INT-05 | 集成 | 端到端 Smoke + 文档指针 | 保留 `pnpm -C apps/desktop sim:smoke:e2e`（HTTP）；Unity 补 region/conformance 自动化测试；落地指针迁入 `03-AI核心`。 | INT-04 | 1d | — |

**M4 核心小计**：约 **16.5 人天**

### M4 降级池（stretch / 进度紧张可砍）
...
| 里程碑 | 任务数 | 预估人天（工作量） | 累计人天 |
|--------|--------|---------|--------|
| M0 Spike | 6 | 9 | 9 |
| M1 最小闭环 | 16 | 33 | 42 |
| M2 多 Agent | 13 | 27 | 69 |
| M3 交互事件 | 11 | 22.5 | 91.5 |
| M4 核心 | 8 | 16.5 | 108 |
| **committed 小计** | **54** | **~108** | **108** |
| M4 降级池 | 6 | 13 | 121 |
| **合计** | **60** | **~121** | **121** |

> **口径说明**：本项目为**单人开发者 + 全程 AI 辅助**，无并行团队。表中「人天」为**工作量估算**（非日历天）；单人推进里程碑**基本串行**（每里程碑内先后端、后其 Unity 前端）。committed 工作量 ~108 人天、降级池 ~13 人天（可砍）。
>
> **日历排期为假设**：~121 人天工作量在单人裸算（~8 人天/周）下约需 15 周；**AI 辅助的目标是压缩单任务墙钟，但提速倍数无真实数据**（见头部「开发阶段声明」），须由 M0/M1 实测校准，**不承诺硬交付周数**。最紧接缝在 M0+M1（Spike 与最小闭环，合计 ~42 人天）——Spike 门未过则里程碑顺延。
...
| # | 风险 | 影响 | 概率 | 应对策略 | 缓冲 |
|---|------|------|------|---------|------|
| R0 | **涌现不成立**：10 个 LLM 居民行为退化（复读/同质化/无聊），"不好看" | MVP 产品价值存疑 | 中 | **SPIKE-06 最早验证**；人设差异化 + 动机层硬约束 + 反思锚定；no-go 则先打磨单场景剧本再扩量 | SPIKE-06 不过则暂停扩量、回炉人设/prompt（动用缓冲池） |
...
| R3 | **3D 性能**：10 NPC + 寻路 + SSE 导致卡顿 | 观测体验差 | 中 | UT-00 FPS 基线；UT-16 Profiler；位置按 tick snapshot 更新 | UT-16 / 降级池 |
...
**缓冲说明**：缓冲来自 **M4 降级池 ~13 人天（可砍）** + 每里程碑 INT 内含 0.5d。最紧接缝在 M0+M1（Spike 与最小闭环），是缓冲最先被动用处；Spike 门未过则里程碑顺延。
...
| 🔴 必须 | UT-00 | Unity + NavMesh + Xbot 能否跑通？（**卖点命门**） | 1 NPC；snapshot 驱动到 `市场`；≥ 30 FPS 粗测 | 同左；跑不通则砍其它保 3D，不降级 2D | 第 1–3 天 |
| ~~SPIKE-01~~ | — | 已废弃 → UT-00 | | | |
...
**Spike 决策门**：
- **UT-00** 或 SPIKE-03 在 **5 天内**未达成功标准 → **暂停 M1 功能开发**，先解决技术方案（最多额外 3 天）。
...
单人开发者串行推进，AI 辅助压缩单任务墙钟。后端优先（产品/技术风险最高），最小闭环后切 Unity 前端，之后逐里程碑交替推进。「阶段」对应里程碑序，非固定日历周。

| 阶段 | 主线 | 说明 |
|------|------|------|
| M0 | SPIKE-02/03/04/06 + UT-00 + DT-01 | Spike 先行，产品/技术风险先打；产出喂入 M1 |
| M1 | DB-01/02 → BE-01~07 + ST-01 → FE-01~05 → INT-01 | 后端最小闭环后切 Unity 前端 |
| M2 | BE-08~14 → FE-06~10 → INT-02 | 多 Agent 核心 |
| M3 | BE-15~18 + BE-22~24 → FE-11~13 → INT-03 | 交互 + 事件 |
| M4 | BE-25/27/28 → FE-14/16/17 → INT-04~05 | 观测 + 回放 + 验收 |
| 降级池 | BE-26/29、FE-15/18/19、ST-02 | 进度紧张可砍 |
...
- Admin 端模拟管理页
- 100+ Agent 规模优化
- 独立 Web 应用（病毒性传播版）——不在本 MVP，但**已确认为中期目标**（Unity WebGL2；见 [愿景 Phase 2](multi-ai-simulation-vision.md)）
