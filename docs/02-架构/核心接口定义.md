...
- 研究类工具（`web_search`/`read_url`）在 `ToolResult.citations` 附带结构化来源（`{url,title,snippet,site}`）；ReAct 循环跨回合聚合去重后经 `citations` SSE 事件下发并持久化到 `messages.citations`，前端渲染为来源卡片。属 UI 元数据，**不回喂模型**。
...
SSE 不走独立连接端点：`POST /v1/conversations/{id}/messages` 的响应体即 `text/event-stream`，执行事件随之推送。SSE 封装见 `api/sse.py`，事件类型详见 [`执行引擎架构设计.md` §十二](../03-AI核心/执行引擎架构设计.md)。
...
→ 见代码：`apps/server/agentcore/db/models.py`（当前已建模 `User` / `Credentials` / `Invite` / `Conversation` / `Message` / `RefreshToken` 六表）
...
> **实现现状**：以下为**目标** ER 图；当前 ORM 仅 §6.1 六表。
...
> **实现现状**：当前 ORM 为六表（§6.1）：`users` / `credentials` / `invites` / `conversations` / `messages` / `refresh_tokens`。下表为完整模块规划。
