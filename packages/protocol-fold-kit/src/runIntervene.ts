 ...
 *
 * 队员跑完之后这两件事**确实做不到**，这没问题；但入口不许因此消失。挖到该在的位置却
 * 发现按钮不见了，用户会以为自己找错了地方——扑空一次就再也不试，最后只敢用够得着的那个
 * 「停止整轮」。所以这里永远给出一句「为什么现在不行」，由调用方渲染成不可用状态 + 原因，
 * 而不是 `null`。
 *
 * 与 {@link turnElapsedMs} / `teamGain` 同理放在 kit 里：桌面协作图节点、桌面右坞 run 详情、
 * 手机队员详情说的必须是同一件事、同一句话。这类判定一旦两端各写一份，很快就会一端说
 * 「已经跑完」、另一端干脆不显示。
 */
