...
async def _finalize_plan_review() -> tuple[PlanReviewSuspension, list[dict]]:
    ...
    sink = EventSink()
    # The delegate's workers run on this provider: s1 → S1OUT (then the plan checkpoints and
    # the turn finalizes). s2 runs only AFTER the resume, on the resume's own provider.
...
async def test_finalized_plan_review_frame_resumes_to_completion(monkeypatch, tmp_path):
    ...
    provider = _ScriptedProvider(
        [[LLMChunk(delta_content="S2OUT")], [LLMChunk(delta_content="最终答复")]]
    )
    _patch_resume_seams(monkeypatch, provider, FileMemoryStore(tmp_path / "memory"))
