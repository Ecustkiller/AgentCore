...
def _synthetic_recording() -> dict:
    ...
    send_leg = [
        ...
        _e("checkpoint_required", _checkpoint_payload(), 70),
        _e("message_end", {"finish_reason": "paused", "usage": _USAGE, "cost": _COST}, 80),
        _e("turn_saved", {}, 85),
    ]
    resume_leg = [
        _e("message_start", {"message_id": "m1", "conversation_id": "conv1"}, 0),
        _e("checkpoint_resolved", {"checkpoint_id": "cp-1", "decision": "continue"}, 5),
...
def test_durable_face_filters_by_disposition():
    ...
    for kept in ("tool_use_start", "tool_use_end", "content_delta", "reasoning_delta",
                 "run_plan", "run_completed", "message_end", "checkpoint_required"):
...
def test_unknown_event_type_is_dropped():
    face = durable_face([{"type": "made_up_event", "payload": {}}])
    assert face == []


def test_non_live_event_types_are_dropped():
    face = durable_face(
        [
            ...
            },
            {
                ...
                "payload": {"execution_id": "e1", "completed": 1, "total": 2},
            },
            {
                "type": "stage_card_required",
                "payload": {"stage_card_id": "sc1"},
            },
            {
                "type": "stage_card_resolved",
                "payload": {"stage_card_id": "sc1", "decision": "start_debate"},
            },
...
def test_e2e_synthetic_recording_cut_passes_turn_conformance(tmp_path: Path):
    ...
    ask = next(i for i in on_disk["projected"]["interactions"] if i["kind"] == "ask_user")
