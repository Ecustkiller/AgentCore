"""Tests for the edit tool (precise, byte-faithful, atomic file edit).
...
async def test_requires_old_string(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "", "new_string": "x"}, _ctx(tmp_path)
    )
...
async def test_rejects_identical_strings(tmp_path: Path):
    (tmp_path / "f.txt").write_text("hi", encoding="utf-8")
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "hi", "new_string": "hi"}, _ctx(tmp_path)
    )
    assert result.success is False
...
def test_identical_edit_fingerprint_collapses_per_path():
    ...
    fp_a = fingerprint_tool_call(
        "edit",
        '{"file_path": "报告.md", "old_string": "AAA", "new_string": "AAA"}',
    )
    fp_b = fingerprint_tool_call(
        "edit",
        '{"file_path": "报告.md", "old_string": "BBB 不同正文", "new_string": "BBB 不同正文"}',
    )
    fp_other = fingerprint_tool_call(
        "edit",
        '{"file_path": "other.md", "old_string": "AAA", "new_string": "AAA"}',
    )
    assert fp_a == fp_b
...
async def test_rejects_path_outside_workspace(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "../secret.txt", "old_string": "secret", "new_string": "x"},
        _ctx(ws),
    )
    assert result.success is False
...
async def test_access_denied_is_lock_not_grant(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "hello", "new_string": "world"},
        ctx,
    )
    assert result.success is False
...
async def test_file_not_found(tmp_path: Path):
    result = await StrReplaceTool().execute(
        {"file_path": "nope.txt", "old_string": "a", "new_string": "b"}, _ctx(tmp_path)
    )
    assert result.success is False
...
async def test_rejects_directory(tmp_path: Path):
    (tmp_path / "d").mkdir()
    result = await StrReplaceTool().execute(
        {"file_path": "d", "old_string": "a", "new_string": "b"}, _ctx(tmp_path)
    )
    assert result.success is False
...
async def test_rejects_binary_file(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "blob.bin", "old_string": "a", "new_string": "b"}, _ctx(tmp_path)
    )
    assert result.success is False
...
async def test_old_string_not_found(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "xyz", "new_string": "b"}, _ctx(tmp_path)
    )
    assert result.success is False
...
async def test_non_unique_without_replace_all_fails(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "x", "new_string": "y"}, _ctx(tmp_path)
    )
    assert result.success is False
...
async def test_ambiguous_receipt_starts_at_hit_and_offers_unique_anchor(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "t.ts", "old_string": needle, "new_string": "expect(calls).toEqual([])"},
        _ctx(tmp_path),
    )
    err = result.error or ""
    assert result.success is False
...
async def test_single_unique_replacement(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "app.py", "old_string": "return a - b", "new_string": "return a + b"},
        _ctx(tmp_path),
    )
...
async def test_preserves_crlf_line_endings(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "win.txt", "old_string": "TARGET", "new_string": "DONE"},
        _ctx(tmp_path),
    )
    assert result.success is True
...
async def test_replacement_inserts_new_text_verbatim(tmp_path: Path):
    ...
    result = await StrReplaceTool().execute(
        {"file_path": "f.txt", "old_string": "old", "new_string": "new value 123"},
        _ctx(tmp_path),
    )
    assert result.success is True
