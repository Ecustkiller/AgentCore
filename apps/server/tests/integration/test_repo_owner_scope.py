...
User-facing data access on the per-user aggregates (conversation / folder)
...
_CONTRACT: dict[str, tuple[str, tuple[str, ...], tuple[str, ...]]] = {
    ...
    "folders.py": (
        "FolderRepository",
        ("get_by_id", "update", "soft_delete", "get_deleted_by_id", "restore"),
        ("get_by_id_unscoped",),
    ),
}
...
def test_user_facing_methods_require_user_id() -> None:
    """Every user-facing read/mutate on the per-user aggregates requires user_id.
