...
from agentcore.memory.rules_injection import _scope_on_demand_user_rules, mutate_user_rule
...
async def test_disputed_entries_leave_the_on_demand_catalog(session_factory):
    ...
    async with session_factory() as session:
        ...
        topics = await load_memory_topics(store, uid, folder_id=None)
        assert topics == []
        assert "先 build 再 deploy" in await store.load(uid, topic_path("部署流程"))

        await repo.set_disputed(rule.id, user_id=uid, disputed=True)
        await repo.set_disputed(topic.id, user_id=uid, disputed=True)

        assert await _scope_on_demand_user_rules(repo, uid, None) == []
        assert await load_memory_topics(store, uid, folder_id=None) == []
...
async def test_disputed_line_leaves_body_and_injection_but_stays_recoverable(
    ...
    async with session_factory() as session:
        ...
        injected = await assemble_injected_rules(inner, repo, uid, folder_id=None)
        assert "用户在腾讯工作" not in injected
        assert "用户住在深圳" not in injected
        ...
        assert "用户在腾讯工作" not in await assemble_injected_rules(
            inner, repo, uid, folder_id=None
        )
...
async def test_full_pool_denies_one_entry_but_pass_continues(
    ...
    async with session_factory() as session:
        ...
        class _Consolidator:
            """Rewrites the always profile (not quota-gated) AND emits a topic op (must not land)."""
    ...
    # Topic ops are not this pass. AI-core rewrite is not gated by the user-rule pool,
    # so a full pool must not emit a quota card for 画像.md.
    async with session_factory() as session:
        repo = DocumentRepository(session)
        topic = await repo.get_memory_note(uid, topic_path("部署流程"), None)
