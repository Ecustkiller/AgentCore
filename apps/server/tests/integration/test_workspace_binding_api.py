...
async def test_binding_requires_auth(client):
    ...
    assert (
        await client.put(
            f"/v1/conversations/{cid}/workspace/binding", json={"root_id": _ROOT}
        )
    ).status_code == 401
    assert (
        await client.delete(f"/v1/conversations/{cid}/workspace/binding")
    ).status_code == 401
...
async def test_bind_foldered_conversation_is_shared_by_siblings(client, make_invite):
    ...
    for cid in (conv_a, conv_b):
        await client.patch(
            f"/v1/conversations/{cid}/folder", json={"folder_id": folder_id}
        )

    # Binding through one conversation writes at the *folder* scope.
    r = await client.put(
        f"/v1/conversations/{conv_a}/workspace/binding", json={"root_id": _ROOT}
    )
    assert r.status_code == 200, r.text
    assert r.json() == {"mode": "local", "scope": "folder", "root_id": _ROOT}
...
async def test_bind_rejects_empty_root_id(client, make_invite):
    ...
    r = await client.put(
        f"/v1/conversations/{conv}/workspace/binding", json={"root_id": ""}
    )
...
async def test_binding_isolation_between_users(client, make_invite, new_client):
    ...
    async with new_client() as other:
        ...
        assert (
            await other.get(f"/v1/conversations/{conv}/workspace/binding")
        ).status_code == 404
        assert (
            await other.put(
                f"/v1/conversations/{conv}/workspace/binding", json={"root_id": _ROOT}
