...
Covers: the auth gate; hub enumeration (folders always; ungrouped only when
non-empty; local marked + carries root_id); the ws-id-addressed file CRUD / move /
dirs / snapshot lifecycle on both a folder and an ungrouped conversation space;
the cloud-vs-local gate (local file/mutation ops 409, read-only snapshot list
stays open); and IDOR / bad-id / foldered-as-conv 404s.
...
async def test_ungrouped_conversation_workspace_by_ws_id(
    client, make_invite, _fs_data_dir
):
    code = await make_invite("INV-WSX-2")
    await _register_and_login(client, code, "wsxuser2")
    conv_id = await _new_conversation(client)
    ws = f"conv:{conv_id}"

    r = await client.put(f"/v1/workspaces/{ws}/files/n.txt", content=b"hi")
    assert r.status_code == 200, r.text
    r = await client.get(f"/v1/workspaces/{ws}/files/n.txt")
    assert r.status_code == 200 and r.content == b"hi"


async def test_enumeration_lists_folders_and_nonempty_ungrouped(
    client, make_invite, _fs_data_dir
):
    code = await make_invite("INV-WSX-ENUM")
    await _register_and_login(client, code, "wsxenum")
    folder_id = await _new_folder(client, "Alpha")
    empty_conv = await _new_conversation(client, "empty")
    filled_conv = await _new_conversation(client, "filled")
    await client.put(f"/v1/workspaces/conv:{filled_conv}/files/f.txt", content=b"x")

    r = await client.get("/v1/workspaces")
    assert r.status_code == 200, r.text
    by_id = {w["ws_id"]: w for w in r.json()["data"]}

    # The folder always lists (a project is a project, empty or not).
    assert f"folder:{folder_id}" in by_id
    assert by_id[f"folder:{folder_id}"]["location"] == "cloud"
    # An ungrouped chat lists only once it actually holds files (F1).
    assert f"conv:{filled_conv}" in by_id
    assert by_id[f"conv:{filled_conv}"]["name"] == "filled"
    assert f"conv:{empty_conv}" not in by_id
...
async def test_enumeration_marks_local_binding(client, make_invite, _fs_data_dir):
    ...
    r = await client.get("/v1/workspaces")
    by_id = {w["ws_id"]: w for w in r.json()["data"]}
    entry = by_id[f"conv:{conv_id}"]  # local spaces list even with no server files
...
async def test_local_workspace_rejects_server_side_ops(
    ...
    conv_id = await _new_conversation(client, "local")
    await client.put(
        f"/v1/conversations/{conv_id}/workspace/binding", json={"root_id": _ROOT}
    )
    ws = f"conv:{conv_id}"
...
async def test_bad_and_foreign_ws_ids_404(client, new_client, make_invite, _fs_data_dir):
    ...
    # File the conversation: its canonical space is now the folder's, so the
    # conv-id form must not resolve (would point at the wrong, unused dir).
    await client.patch(
        f"/v1/conversations/{conv_id}/folder", json={"folder_id": folder_id}
    )
