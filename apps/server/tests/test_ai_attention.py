...
def test_only_blocking_kinds_map_to_an_attention_kind():
    """Progress surfaces stop the badge, not the turn — they must not signal."""
    for blocking in (
        "approval",
        "escalation",
        ...
        assert attention_kind_of(blocking) is AttentionKind(blocking)

    for non_blocking in ("client_tool", "stage_card", "question_posted", "nonsense"):
        assert attention_kind_of(non_blocking) is None
