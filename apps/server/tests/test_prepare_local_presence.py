...
def test_error_fields_for_three_prepare_abort_messages():
    for message in (LOCAL_DESKTOP_OFFLINE, LOCAL_ROOT_NOT_HELD, LOCAL_CHANNEL_DEAD):
...
async def test_prepare_aborts_desktop_offline_skips_llm(monkeypatch):
    ...
    monkeypatch.setattr(
        "agentcore.runtime.pipeline.prepare.load_project_catalog", _empty_catalog
    )
