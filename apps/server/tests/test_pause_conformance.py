...
class _ResolveBridge:
    """A ClientRequestBridge stub whose ``suspend`` settles immediately.

    Mirrors the live path's two observable side effects without an event loop wait:
    it fires ``on_suspended`` (which emits the ``checkpoint_required`` display event,
    exactly as the registry does once the turn is parked) and returns the user's
    decision so the captain loop runs on to completion. The transcript/journal we
    assert on are snapshotted by the saver BEFORE this returns, so HOW it resolves
    (stop here) does not affect the pause snapshot under test.
    """
...
async def test_pause_journal_projects_to_captain_transcript():
    ...
    ask_tool = AskUserTool(
        sink=sink,
        conversation_id="c1",
        registry=_ResolveBridge(CheckpointDecision.STOP),
        timeout_seconds=1.0,
        captain_run_id="cap",
...
async def test_plan_review_pause_journal_projects_to_captain_transcript(monkeypatch):
    ...
    # The captain's delegate runs its workers on this scripted provider: s1 → S1OUT (then
    # the plan checkpoints), s2 → S2OUT (after resume). Separate from the captain provider.
    worker_provider = _ScriptedProvider(
        [[LLMChunk(delta_content="S1OUT")], [LLMChunk(delta_content="S2OUT")]]
    )
