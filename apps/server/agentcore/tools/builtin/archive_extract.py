"""archive extract runner — unzip a workspace zip into a destination directory.

Dispatched by ``archive(action=extract)``. Local and cloud both write member
bytes through ``WorkspaceBackend.write_bytes``. Zip-slip protection reuses
``storage._archive`` helpers.
"""
...
async def run_extract(arguments: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    lines.append("【验真】请以本回执路径确认落盘；可用 glob 抽查目标目录。")
