...
_PROFILE_UPDATED_OPEN = "<project_profile_updated>"
_PROFILE_UPDATED_CLOSE = "</project_profile_updated>"
...
class UpdateProjectProfileTool:
    ...
    # so the next worker LLM round sees the new project profile this turn.
    ...
    async def execute(self, arguments: dict[str, Any], context: ToolContext) -> ToolResult:
        ...
        if conflict:
            return ToolResult(
                ...
                error="conflict",
                display={"written": False, "conflict": True, "kind": "project_profile"},
            )
        if not ok or not resulting.strip():
            return ToolResult(
                ...
                display={"written": False, "kind": "project_profile"},
            )
        ...
        if topics:
            try:
                topic_paths = await write_project_topics_replace(
                    store=store,
                    user_id=context.user_id,
                    folder_id=self.folder_id,
                    topics=topics,
...
def build_update_project_profile_tool(
    *,
    folder_id: str | None = None,
    store: MemoryStore | None = None,
    ...
    return UpdateProjectProfileTool(
        folder_id=folder_id,
        store=store,
