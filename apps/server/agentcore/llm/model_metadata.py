...
three flags the frontend renders. The ``vision`` bit is **not** stored here; it
is overlaid from :func:`agentcore.llm.image_accept.model_accepts_images` so the
catalog and native-multimodal routing share one vendor contract. Context length
is the gateway window in tokens (catalog display **and** compaction near-top =
80% of this number). Prefer the SKU the user actually hits, not the model's
advertised native max: a ``-free`` row that a gateway caps at 200K must not
inherit a sibling's 1M.
"""
...
# vendor / non-vision capabilities / context — but display_name always gets a · qualifier so
...
_METADATA: dict[str, ModelMeta] = {
    ...
    "deepseek-v4-flash-vision-exp": ModelMeta(
...
def _derive_capabilities(key: str) -> frozenset[str]:
    """Best-effort non-vision tags from id keywords (conservative — omit if unsure).

    ``vision`` is never derived here; :func:`model_accepts_images` owns that bit.
    """
    if any(tok in key for tok in ("reason", "think", "-r1", "o1", "o3", "o4", "-r-")):
        return frozenset({CAPABILITY_REASONING})
    return frozenset()
...
def _family_variant_meta(family: ModelMeta, key: str, family_key: str) -> ModelMeta:
    ...
    badges. They also do **not** copy ``vision``; that bit is overlaid from
    :func:`model_accepts_images`. The · qualifier keeps labels unique when badge
    is absent.
    """
    ...
    return ModelMeta(
        ...
        capabilities=family.capabilities - {CAPABILITY_VISION},
        context_length=family.context_length,
        badge=None,
    )


def _overlay_vision(meta: ModelMeta, model_id: str) -> ModelMeta:
    """Stamp the vendor-contract vision bit; never trust stored / inherited tags."""
    caps = set(meta.capabilities)
    caps.discard(CAPABILITY_VISION)
    if model_accepts_images(model_id):
        caps.add(CAPABILITY_VISION)
    new_caps = frozenset(caps)
    if new_caps == meta.capabilities:
        return meta
    return ModelMeta(
...
def model_metadata_for(model_id: str) -> ModelMeta:
    ...
    vendor guess, keyword-inferred non-vision capabilities) so the catalog stays
    complete. Family-prefix hits keep vendor / non-vision caps / context but
    append a · qualifier to ``display_name`` so dated / channel siblings stay
    distinguishable in pickers. ``vision`` always comes from
    :func:`model_accepts_images`.
    """
    key = _normalize(model_id)
    if not key:
        return _overlay_vision(
            ModelMeta(display_name=_humanize(model_id), vendor="其他"),
            model_id,
        )
    exact = _METADATA.get(key)
    if exact is not None:
        return _overlay_vision(exact, model_id)
    best_key = _longest_family_key(key)
    if best_key is not None:
        return _overlay_vision(
            _family_variant_meta(_METADATA[best_key], key, best_key),
            model_id,
        )
    return _overlay_vision(
        ModelMeta(
