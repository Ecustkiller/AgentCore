...
Lives in ``conversation`` (composition root) so ``runtime`` only holds the
``recover_hooks`` injection seam. ``main`` binds this factory at lifespan.
"""
...
from agentcore.db.repositories import ConversationRepository, TableRepository
