...
class MemoryUpdateRepository:
    ...
    async def record(
        self, *, conversation_id: str, user_id: str, items: list[dict]
    ) -> MemoryUpdateRow:
        """Persist one pass's applied changes and return the stored row (with id/created_at).

        The caller (consolidation) records this ONLY when a pass changed a file, so the
        table holds just real updates. Commits its own unit of work — it rides the offline
        pass, not a request transaction.
        """
        row = MemoryUpdateRow(conversation_id=conversation_id, user_id=user_id, items=items)
