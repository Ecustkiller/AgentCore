...
Three sources (skill / rule / memory) each implement :class:`Consultable`.
...
Namespace priority on collision: skill → rule → memory (deterministic). Shadowed
names log ``consult.name_shadowed``.
"""
...
_SOURCE_PRIORITY: tuple[str, ...] = ("skill", "rule", "memory")
...
class MergedConsultSource:
    """Skill → rule → memory merge; prompt directory and tool fetch share this instance."""
...
def build_merged_consult_source(
    *,
    ...
    return MergedConsultSource(skill=skill, rule=rule, memory=memory)
