...
* ``closing_posture_core`` — tier & posture-A/C/draft + honesty rework
* ``closing_posture_resume`` — resume / continuity join & steer
* ``closing_posture_ceiling`` — hard-ceiling steer / banner / verdict downgrade
* ``closing_posture_ceo_mutation`` — CEO mutation / disk-landing claims
* ``closing_posture_cloud_web`` — cloud web verify latch + enforce
* ``closing_posture_cutoff`` — cutoff / token_budget latch + enforce
* ``closing_posture_write_ownership`` — write-ownership latch + enforce
* ``closing_posture_browser`` — browser assemble/tool-success latch
* ``closing_posture_over_seat`` / ``empty_handoff`` / ``cancel_zero`` — storm latches
* ``closing_posture_verify_budget`` — verify-budget latch
* ``closing_posture_hollow`` — hollow teach / in-progress claims
* ``closing_posture_b1`` — prepare clear_b1 + cross-latch probes
...
from agentcore.runtime.closing_posture_b1 import clear_b1_closing_latches
from agentcore.runtime.closing_posture_browser import (
...
from agentcore.runtime.closing_posture_cancel_zero import (
    clear_cancel_zero_output,
    note_cancel_zero_output,
    turn_has_cancel_zero_output,
)
from agentcore.runtime.closing_posture_ceiling import (
...
from agentcore.runtime.closing_posture_cloud_web import (
...
from agentcore.runtime.closing_posture_cutoff import (
...
from agentcore.runtime.closing_posture_empty_handoff import (
...
from agentcore.runtime.closing_posture_over_seat import (
    clear_over_seat_reject,
    note_over_seat_reject,
    turn_has_over_seat_reject,
)
from agentcore.runtime.closing_posture_resume import (
...
from agentcore.runtime.closing_posture_verify_budget import (
...
from agentcore.runtime.closing_posture_write_ownership import (
