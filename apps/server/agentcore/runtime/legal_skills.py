...
不带 #rN，也绝不可指向未登记的 id（编造引用会被交付前核验拦截、打回重写）。
...
（用户不是带着案卷来做律师接案评估）时：【停止】按本 skill 编排，立即改 `consult_skill(deep_multi_lens_research)` \
并按其指引走四路调研（形态贴合时可选用 playbook=\"multi_lens_research\"）。本 skill 只服务律师作业的接案评估 /\
诉讼策略——商标舆情类「要开庭」≠ 律师接案。
...
LEGAL_SKILLS: tuple[SystemSkill, ...] = (
    SystemSkill(
        ...
        summary=(
            "律师作业：写/打磨民事答辩状文书——对方律师作战室"
            "（先 red_team 对抗再核验收口；非公共事件多维取证）"
        ),
    ...
    SystemSkill(
        ...
        summary=(
            "律师作业：民事接案评估/诉讼策略——"
            "【除外】公共品牌/舆论/商标争议的模拟法庭（需多维取证）先走 deep_multi_lens_research；"
            "其余先对抗后研判：debate 原被告 → 法官研判 + 法条核验 → 人审落盘"
            "（文书与诉讼路径，非公共事件多维研判）"
        ),
