...
【法条引用带来源角标 [n]】核验 worker 检索到的法条权威来源会自动编入本回合「来源卡」，并随核验结果以\
『[来源编号] [n]=url』给到你；你在终稿正文里引用每条【已核验】法条时，用对应的 `[n]` 角标标注，读者点角标即可\
溯源到那张来源卡（这正是「玻璃箱可辩护」的落地）。仍标『[待核验]』、未取到权威来源的法条【不要】编角标——宁可\
不带 [n]，也绝不可指向不存在的来源（编造的角标会被交付前核验拦截、打回重写）。
...
LEGAL_SKILLS: tuple[SystemSkill, ...] = (
    ...
    SystemSkill(
        ...
        summary=(
            "对民事案件做「原告 / 被告 / 法官」三方视角研判（接案评估 或 诉讼策略）：debate 原被告"
            "对抗 → delegate 中立法官研判 + 法条核验 → 人审收口、落盘研判文件（含反幻觉硬约束）"
        ),
