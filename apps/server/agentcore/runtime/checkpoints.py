...
AskCheckpointIntent = Literal["decision"]

_ASK_CHECKPOINT_INTENTS: frozenset[str] = frozenset({"decision"})
...
class CheckpointDecision(StrEnum):
    ...
    ADJUST = "adjust"  # leftover; ask_user rejects
...
class CheckpointResponse:
    ...
    ``note`` is an optional remark for ``STOP`` / ask continue; empty for ``TIMEOUT``.
    ``selected`` holds the option(s) the user picked from the CEO's ``options``
    menu — one for a single-select ask, several when the ask is ``multiple`` —
    and is a first-class part of the answer (no longer folded into ``note``), so
    ``CONTINUE`` carries the pick too. Empty when the ask offered no options or
    the user chose none.
    """
