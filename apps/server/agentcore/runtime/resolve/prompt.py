...
——你都没有对应工具，必须 `delegate` 交给 worker。这是刻意分工。worker 的工具集不是无所不能：\
按本回合环境装配，以 `<workspace_context>` 的「本回合执行能力」行为准——`code_execute=未装配` \
时 worker 同样【没有】执行环境（能写文件、不能运行代码，也不能生成需运行程序才能产出的二进制 / \
可播放文件），委派前先按此对齐任务与交付形态，不要按「worker 什么都能跑」规划。
