...
**B. 单 lead + 嵌套扇出**——根只交成果级目标·约束·验收，由 lead 再 `delegate` 拆子任务（深度上限内）。\
...
（scope 必填；拆缝已清则填 `modules` 少扇出常 2–4 一次专班；\
...
手段优先入口文件（Git 可用则看进度），够用即停；收工须 handoff 短摘要，\
...
【跨项目 / 空壳 kickoff】用户要同时推进多项目（「同时开发 A 和 B」等）：先\
...
各 task 填已解析 `target_folder_id`；【禁止】CEO 串行翻两空目录代替派工。\
【禁止】用 `external_mount_readonly` 乱挂文档/桌面冒充开发双仓（挂载=区外只读，与写盘桌正交）。\
