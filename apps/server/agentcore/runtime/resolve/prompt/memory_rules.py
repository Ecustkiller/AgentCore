"""Always-on ``<rules>`` injection template (read-side equal-authority join)."""

# Unique owner for「题材偏好不得改路由」— the flat ``<rules>`` block formats this in.
...
def _format_rules(rules_markdown: str | None) -> str | None:
    """Wrap always-on entries into one ``<rules>`` block, or None if empty."""
