...
根 depth=0、无具名 playbook、本批恰好 1 task、非 finalize、显式写工程
（``form=files`` 或 ``requires_files=true``；form 省略不算）、且无切片钉 →
记一次软告警。不拒收、不改图。路径 B（该 lead 嵌套扇出）为合法等价编制，文案须明示。

切片钉（白名单，任一即豁免）：非空 ``artifacts`` / 非空 ``artifact_dir`` /
``min_length>0`` / 非空 ``required_sections`` / 本 task ``checkpoint_after``。
"""
...
def _explicit_write_engineering(task: dict[str, Any]) -> bool:
    """True only when CEO explicitly marked files delivery (omit ≠ files)."""
    d = _deliverable(task)
    if d.get("form") == "files":
        return True
    return d.get("requires_files") is True
...
def root_slice_honesty_soft_message() -> str:
    return (
        ...
        "（artifacts、artifact_dir、min_length、required_sections 等），"
