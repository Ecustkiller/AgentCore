...
- 同回合二次派发 / adopt 热图 → 合入同一 ``execution_id``（``append_to``，不写 prev）
- 跨回合已收口图 → 本回合新图 + ``prev_execution_id`` 链（不 divert）
    ...
    *,
    ...
    if append_to and append_to.lower() == "latest":
        ...
        if active is not None and active.active and (
            _is_same_host_turn_append(active, message_id)
    ...
    if append_to:
        ...
        if (  # noqa: SIM102
            ...
            if _is_same_host_turn_append(active, message_id) or (
                active.execution_id == (context_execution_id or "")
            ):
                append_to = None
    ...
    if host_plan_for_append is None and not append_to:
        active = coordination_session.active_coordination(context_execution_id)
        bound_eid = (context_execution_id or "").strip()
        if (
            active is not None
            and active.active
