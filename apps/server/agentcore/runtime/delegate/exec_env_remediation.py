...
def exec_env_remediation_zh(
    *,
    ...
    if kind == "delivery_action":
        ...
        if _sidecar_engine():
            return (
                ...
                "**不要**再引导「先在云上做」，也不要等本机沙箱恢复。"
            )
        return (
            "本回合已是云端会话，但云端执行沙箱未装配"
            f"{failure_clause}——**不要**再引导「先在云上做」。"
    ...
    if kind == "capability_office":
        ...
        # Word/PDF 有 md_export，与沙箱正交——别顺手一起报做不到。
        docx_clause = (
            "【注意·别扩大缺口】`.docx` / `.pdf` **不在**本缺口内："
            "确定性导出器 `md_export` 与执行沙箱无关"
            "（本回合工具表有这把即可用，不看 `run`），"
        ...
        if local:
            return (
                ...
                f"{docx_clause}"
                "【禁止】再派「写脚本 / 跑脚本」空转，也【禁止】再声称本回合工具表已有 run；"
                "请立即发 ask_user 卡说明缺口，并**推荐**引导 Composer「先在云上做」"
    ...
    return (
        ...
        "【禁止】再引导「先在云上做」。收尾如实标缺口，或 ask_user："
...
def cloud_exec_unavailable_delivery_action(backend: Any) -> dict[str, str]:
    ...
    copy no longer defaults to 「再先在云上做」 when already on a cloud desk.
    """
    return {
