...
"非阻塞却不给默认"等于偷偷瞎猜），【必须】写 `unlocks`（这个答案回来解锁哪批活；不写则拒调；纯知会写\
正文勿走本工具）。不挂起；按已声明默认继续**或**声明后半等人——两种都须写明默认。\
抛问同时只派答案影响不到的活；`unlocks` 那批等答案回来再追加。收口见主提示【非阻塞问·压单】。\
问题会作为一条不阻塞的提示呈现给用户，ta 若回复会作为新消息在后续轮次到达，你届时再派 `unlocks` 那批。\
判准：猜错只是小返工 / 能平滑纠偏 → 非阻塞；猜错会让产物大面积作废 / 不可逆 → 仍用 `blocking=true` \
暂停等答复。别拿它替代真正该阻塞的关键岔路，也别为能完全自行决定的小事用它（那只需正文标注一句）。
