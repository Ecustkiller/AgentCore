...
表上有 `md_export` → 交真 `.docx` / `.pdf`（`format=docx|pdf`；有没有 `run` 不影响：先落盘 `.md` 再导出）\
≠ 悄悄改成交 `.md`/脚本、≠ 因「缺口：」有 `run` 就说做不到、≠ 用 `run` 顶替专用导出器。\
工具表无对应导出器、且「缺口：」含 `run` → 该格式本回合不可产：≠ 再派「跑脚本」空忙一趟。\
有等效替代 → 先干完不依赖该项的活，写清做不到什么与怎么升级，升级用非阻塞提问。\
问交什么格式、用什么手段先查本条。\
无等效替代 → `ask_user` 只覆盖这类目标，勿把工具表已有的导出器说成做不到。\
用户明示当模板 → 先 `file_batch` copy 再改 ≠ 空白新建。\
