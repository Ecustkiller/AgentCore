...
from agentcore.runtime.events import EventSink, board_op_required
...
class BoardChannel:
    ...
    are available and bound to one ``board_id``. ``request`` is the only entry point;
    ``BoardOpsTool`` builds the JSON-safe op list and interprets the returned value.
    """
    ...
    async def request(
        self, ops: list[dict[str, Any]], *, summary: str = ""
    ) -> dict[str, Any]:
