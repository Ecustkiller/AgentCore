...
in-process call metering, main-turn finalize, and handoff job persistence.
Request / finalize path may write synchronously; on failure the caller enqueues
here. A background drain writes via the **telemetry** pool so spend never
contends with content-write connections (as-built: 成本配额 §三).

Idempotency: call details use stable ``call_id``; run aggregates use ``run_id``.
``CostEventRepository`` uses ``ON CONFLICT DO NOTHING`` (finalize) or
``DO UPDATE`` (proxy materialize), so drain retries never double-bill.
