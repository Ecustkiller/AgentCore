...
1. ``runtime.events.types.EventType`` values == keys of ``SSEPayloadMap`` in
   ``packages/contract-types/src/events.ts``.
2. ``eventTypes.generated.ts`` union == ``EventType`` (catches stale generated file).

Payload *shapes* stay hand-maintained; this script only guards the discriminant
set from silent drift.
"""
...
EVENTS_TS = ROOT / "packages" / "contract-types" / "src" / "events.ts"
GENERATED_TS = ROOT / "packages" / "contract-types" / "src" / "eventTypes.generated.ts"
...
def main() -> None:
    ...
    events_text = EVENTS_TS.read_text(encoding="utf-8")
    payload_keys = _parse_payload_map_keys(events_text)
    ...
    print(
        ...
        f"SSEPayloadMap + generated union aligned)"
    )
