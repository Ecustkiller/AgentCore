...
Run from ``apps/server`` (reads ``SEARXNG_URL`` from settings/env)::
    ...
    uv run python scripts/archive/probe_search_engines.py
    uv run python scripts/archive/probe_search_engines.py --query 贝叶斯定理 --query 深圳天气
    uv run python scripts/archive/probe_search_engines.py --engines baidu,360search   # only these
    uv run python scripts/archive/probe_search_engines.py --url http://127.0.0.1:18888 # override host
