...
Run from ``apps/server`` (uses the same DB + a user's own BYOK key)::
    ...
    uv run python scripts/run_research_probe.py
    uv run python scripts/run_research_probe.py --message "..."   # custom prompt
    ...
    async with async_session_factory() as session:
        row = (
            await session.execute(
                select(UserLlmKey).where(UserLlmKey.api_key_enc.isnot(None)).limit(1)
            )
        ).scalar_one_or_none()
        if row is None:
            raise SystemExit(
