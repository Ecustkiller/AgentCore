...
从 apps/server 跑::
    ...
    uv run python scripts/probe_ask_gate.py
...
async def _build_ceo_context(mode: str):
    ...
    _delegate, _revise, _debate, chat_tools = _assemble_ceo_toolset(
        llm=provider,
        sink=EventSink(),
    ...
    return provider, chat_profile, ceo_prompt, tool_defs, sorted(ceo_tool_names), chat_tools, ctx


async def _run_one(
    provider, profile, ceo_prompt: str, tool_defs, chat_tools, ctx, user_message: str, max_rounds: int
):
    ...
    for r in range(max_rounds):
        request = build_request(profile, messages, tools=tool_defs, tool_choice="auto", stream=False)
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    provider, profile, ceo_prompt, tool_defs, names, chat_tools, ctx = await _build_ceo_context(
        args.mode
    )
    print(f"模型档: {args.mode}  chat.model={profile.model}  thinking={profile.thinking}  最多 {args.rounds} 轮")
    ...
    for sc in selected:
        ...
        for i in range(args.samples):
            cls, detail, trail, rounds, usage = await _run_one(
                provider, profile, ceo_prompt, tool_defs, chat_tools, ctx, sc.user_message, args.rounds
            )
