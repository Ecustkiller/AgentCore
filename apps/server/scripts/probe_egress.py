"""出网健康探针 —— DNS 解析 + SSRF 判定 + read_url 端到端真读，一处验证联网取证层是否通。

诊断场景：辩论 / web_search / read_url「无据可依」，或迁到新机器 / 服务器时先验证出网。
...
从 apps/server 下跑：
    ...
    uv run python scripts/probe_egress.py                 # 默认目标（CN + 国际各若干）
    uv run python scripts/probe_egress.py https://a.com https://b.com   # 自定义目标
...
async def _probe_one(url: str) -> dict:
    ...
    if block is None:
        ctx = ToolContext(
            ...
            run_id="probe",
            agent_id="probe",
            backend=None,  # read_url 只用 conversation_id，不碰 backend
            user_id="probe",
            conversation_id="",
        )
        try:
            res = await ReadUrlTool().execute({"url": url, "max_chars": 800}, ctx)
            if res.success:
...
async def main() -> None:
    ...
    print("=" * 78)
    print("出网健康探针（DNS + SSRF + read_url 端到端）")
    print("=" * 78)
