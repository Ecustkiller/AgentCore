/**
 * 流式 Markdown 的**逐块切分**（对齐桌面 chat/streamingMarkdown 行为；本端自有副本）。
 *
 * 回合流式期间，`Markdown` 每收到一批 delta 就重渲染一次；若每次都把累积的全文重新解析，
 * 整轮是 O(n²)。这里把内容切成**一串顶层块**：每个已写完的块各自记忆化、只解析一次并永久冻结，
 * 仅最后的「在写尾块」随每批 delta 重解析——整轮降到 O(总量)。
 *
 * 安全性：只在**栅栏代码块之外**、且**下一非空行不是列表/引用/缩进续行**的空行处切分，因此
 * 每一块各自都是「独立合法的 Markdown」。调用方对**收尾后的最终内容仍按整篇渲染一次**，
 * 故本切分保守漏掉的跨块引用会在终态被正确解析。
 */

/** 续行：列表项 / 有序列表项 / 引用 / 缩进（≥2 空格或 Tab）。绝不在这类行之前切分。 */
const CONTINUATION = /^(?:\s*(?:[-*+]|\d+[.)])\s|>|\s{2,}\S|\t)/;
/** 栅栏围栏：``` 或 ~~~（长度 ≥ 3），允许少量前导空格。 */
const FENCE = /^\s*(`{3,}|~{3,})/;

/**
 * 把流式内容切成**一串顶层块**，恒满足 `blocks.join("") === content`。
 *
 * 空串返回 `[]`；无完整块时整体作为唯一一块返回 `[content]`。仅供流式期调用；
 * 收尾后整篇渲染一次，无需切分。
 */
export function splitMarkdownBlocks(content: string): string[] {
  if (content === "") return [];
  if (!content.includes("\n\n")) return [content];

  const lines = content.split("\n");
  const starts = new Array<number>(lines.length);
  let off = 0;
  for (let i = 0; i < lines.length; i++) {
    starts[i] = off;
    off += lines[i].length + 1;
  }

  const cuts: number[] = [];
  let last = -1;
  let inFence = false;
  let fenceChar = "";
  for (let i = 0; i < lines.length; i++) {
    const fence = FENCE.exec(lines[i]);
    if (fence) {
      if (!inFence) {
        inFence = true;
        fenceChar = fence[1][0];
      } else if (fence[1][0] === fenceChar) {
        inFence = false;
        fenceChar = "";
      }
      continue;
    }
    if (inFence) continue;
    if (lines[i].trim() !== "") continue;

    let j = i + 1;
    while (j < lines.length && lines[j].trim() === "") j++;
    if (j >= lines.length) break;
    if (CONTINUATION.test(lines[j])) continue;
    const cut = starts[j];
    if (cut !== last) {
      cuts.push(cut);
      last = cut;
    }
  }

  if (cuts.length === 0) return [content];
  const blocks: string[] = [];
  let prev = 0;
  for (const cut of cuts) {
    blocks.push(content.slice(prev, cut));
    prev = cut;
  }
  blocks.push(content.slice(prev));
  return blocks;
}
