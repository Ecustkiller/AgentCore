...
/*
 * Frame evaluator for collaboration-graph segments: given a graph spec
 * (nodes / edges / positions / wave schedule / streaming copy / optional debate
 * axis) and the current scene-local frame, produce ReactFlow node/edge state
 * for GraphStage.
 *
...
export function buildGraphState(
  ...
  if (!spec.debate) {
    return { nodes, edges, debate: null };
  }
