/*
 * Still diagrams for screenshots / covers. Ported from the product manual
 * (`MechanismContent.tsx`) and rendered via Remotion `renderStill` (real
 * AgentNode / EndpointNode through GraphStage). The manual stays tuned for
 * in-app reading (fit-to-width shrink, 520px cap), which is wrong for these crops.
 *
 * Curated set (each is a distinct collaboration shape):
 *   fanout   并行扇出      — 默认差异点（一句话 → 真团队并行）
 *   debate   圆桌辩论      — 用户点名才开的形态，不是默认高潮
 *   nested2  多层嵌套      — 委派分小队、深度 2
 *   bigteam  团队协作全景  — 扇出 + 汇聚 + 辩论 + 嵌套一图
 * App 外壳合成图见 `scenes/AppShellStill.tsx`（默认样图是并行扇出，不在 STILL_DEFS 里）。
 *
...
export const STILL_DEFS: StillDef[] = [
  ...
  // ② 圆桌辩论 — 点名才开的形态，不是默认 GraphRun。主持人定题 → 正反两营对射 → CEO 裁决。
  {
    id: "debate",
    label: "圆桌辩论",
    layout: "leftright",
  ...
  // 沙漏：辩论产出一个结论而非两条工作线，裁决再驱动整波执行（带状布局保证正反两营不交叉）。
  {
    ...
    nodes: [
      ...
      agent("arch", "技术架构", "running", {
        task: "统筹技术骨架与系统边界",
        outputPreview: "正在权衡怎么编排队员与共用文件夹…",
        toolCount: 1,
      }),
