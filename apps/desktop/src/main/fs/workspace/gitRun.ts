 ...
 * ``create`` (default true) mkdir so a missing project subpath can still run
 * git (clone dest lives under cwd); UI SCM passes ``create: false`` and treats
 * a missing desk as no repo. Never fall back to the container root when a
 * subpath is set.
 */
