...
/** Default project rows in draft「在哪工作」before「查看全部」. */
export const DRAFT_PROJECT_PREVIEW_LIMIT = 5;
...
/** Best-effort「最近活跃」: max conversation `updatedAt` in that project. */
...
/**
 * Draft chip project list: recent-first; default cap {@link DRAFT_PROJECT_PREVIEW_LIMIT};
 * filter / expand shows full match set; selected project always stays visible when capped.
 */
export function visibleDraftProjects(opts: {
  folders: FolderMeta[];
  conversations: { folderId?: string | null; updatedAt: string }[];
  query: string;
  expanded: boolean;
  ...
  if (filtering || opts.expanded || matchCount <= DRAFT_PROJECT_PREVIEW_LIMIT) {
    return {
      visible: matched,
      matchCount,
      canExpand: !filtering && matchCount > DRAFT_PROJECT_PREVIEW_LIMIT,
      hiddenCount: 0,
    };
  }

  let visible = matched.slice(0, DRAFT_PROJECT_PREVIEW_LIMIT);
