...
/** Answer text so the CEO sees which folder was registered (same conversation). */
export function formatRegisterLocalFolderAnswer(
  optionLabel: string,
  folderName: string,
): string {
  return `${optionLabel}（${folderName} · 已登记为本机文件夹，仍在本对话）`;
}

export type PickAndRegisterLocalFolderResult =
 ...
 * current session's ``folder_id``. Distinct from {@link pickAndOpenLocalFolder}
 * (open as birthplace + new session).
 */
export async function pickAndRegisterLocalFolder(opts?: {
  notifyOnFailure?: boolean;
}): Promise<PickAndRegisterLocalFolderResult> {
  ...
  try {
    ...
    const message =
      e instanceof Error ? e.message : "登记本机文件夹失败，请重试";
    if (notifyOnFailure) {
      notifyError(e, "登记本机文件夹失败");
    }
