...
import {
  DRAFT_PROJECT_PREVIEW_LIMIT,
  sortFoldersByRecentActivity,
  visibleDraftProjects,
} from "../draftWorkspaceProjects";
...
describe("visibleDraftProjects", () => {
  ...
  it("caps to preview limit when collapsed and unfiltered", () => {
    ...
    expect(r.visible).toHaveLength(DRAFT_PROJECT_PREVIEW_LIMIT);
  });
  ...
  it("keeps a selected project visible when outside the preview", () => {
    const r = visibleDraftProjects({
      folders,
      conversations,
      query: "",
