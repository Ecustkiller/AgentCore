/**
 * Ctrl+R after 开做：leftover 编制确认卡不得复活为可点壳，协作图仍按 execution 画。
 *
 ...
 * （leftover team_preview 不画可点壳）→ 图走 shouldShowTeamGraph
 * （只看 IX 原始 status + execution.runs，不复用 settled 判据）。
 *
 * leftover team_preview journal（checkpoint_id=tp1, execution_id=exec1）。
 */
...
/** leftover team_preview journal 用的工人 / 工具 / 编制。 */
const WORKERS = [
...
describe("Ctrl+R after 开做 — leftover 不画可点壳 + 图仍出", () => {
  it("坏序：cache 开做前快照 + GET 仍是 paused 投影（无 *_resolved）→ leftover 不画可点壳、图随 run_plan 出", () => {
    seedUser();
    // 磁盘 cache 来自上次 GET（开跑前挂起窗）。adopt 不跑 toMessage，IX 空。
