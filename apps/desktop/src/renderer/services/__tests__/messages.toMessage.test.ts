...
function row(over: Partial<BackendMessage> & Pick<BackendMessage, "id" | "role">): BackendMessage {
  return {
    conversation_id: "c1",
    content: "hello",
...
describe("toMessage (reload hydrate)", () => {
  it("stamps serverMessageId = row id on assistant so resume guards match live", () => {
    const msg = toMessage(row({ id: "srv-msg-1", role: "assistant", content: "ok" }));
  ...
  it("does not stamp serverMessageId on user rows", () => {
    const msg = toMessage(row({ id: "srv-user-1", role: "user", content: "hi" }));
