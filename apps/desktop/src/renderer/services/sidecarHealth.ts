 ...
 *     `bad` 带 TTL——过期后允许再探，避免每发都 spawn。返回的 `probed` 区分「本次真探活」
 *     与「命中缓存」，便于日志 / 诊断。
...
/** 本会话对某 sidecar 目标的健康结论：`ok` 可走 / `bad` 报错不走云 / `unknown` 尚未探活。 */
...
/**
 * 标记某 sidecar 目标本会话「环境起不来」（探活失败、或启动期失败）。
 *
 * 标记后 `probeSidecar` 对该根在 TTL 内命中 `bad` 缓存（`probed:false`）；过期后允许再探。
 * `detail` 留在缓存里，命中后再发仍能打出可读诊断（不必等 TTL 再探）。
 */
