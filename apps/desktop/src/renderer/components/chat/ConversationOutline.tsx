 ...
 * (scroll + flash) via the shared `focusMessage`. Complements 会话内查找 and 消息永久链接:
            ...
            {turns.map((t, i) => (
              <li key={t.id}>
                <button
                  type="button"
                  onClick={() => jump(t.id)}
                  className="flex w-full items-start gap-2 rounded-md px-2 py-1.5 text-left text-sm text-foreground hover:bg-accent"
                >
