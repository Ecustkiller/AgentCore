 ...
 * {@link splitStreamingMarkdown} 是其上「冻结前缀 + 尾块」两分视图的薄封装（向后兼容）。
 *
...
/**
 * 把流式内容切成 `{ stable, tail }`，恒满足 `stable + tail === content`：`tail` 是最后一块
 * （在写尾块），`stable` 是其前所有已写完块。{@link splitMarkdownBlocks} 的两分视图封装。
 *
