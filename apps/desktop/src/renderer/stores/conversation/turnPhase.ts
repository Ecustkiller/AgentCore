 ...
 *
 * `workspace_op_required` / `host_op_required` **故意不**放行：stopping/terminal
 * 由 dispatch 失败 settle（诚实 fail 信封），避免静默 drop → 服务端 TimeoutError
 * 冲 sticky channel-dead；也不无差别放开其它流式事件。
 */
