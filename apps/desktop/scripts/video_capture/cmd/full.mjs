/**
 * full — director-driven tape capture for any demo tape.
 *
 * Invoked via: node scripts/video_capture.mjs full --tape <id> [--out]
 *
...
import { desktopDir, distWeb, resolveCapturePaths, loadCreds, captureEnv } from "../shared/paths.mjs";
...
function mayOverwriteStill(id) {
  const raw = (captureEnv("OVERWRITE", "") || "").trim();
  if (!raw) return false;
...
async function shot(page, absPath, { id, force = false } = {}) {
  ...
  if (!force && !mayOverwriteStill(stillId)) {
    try {
      ...
      console.log("SKIP existing still (set VIDEO_OVERWRITE to replace)", stillId);
      ...
      if (/无法连接后端|服务暂时不可用/.test(bodyText)) {
        throw new Error(
          `Webapp cannot reach backend (CORS origin / API-host mismatch). ` +
            `Use the same host for VIDEO_API and VITE_API_URL (localhost, not 127.0.0.1). Page: ${bodyText}`,
        );
      }
      throw e;
    }
...
export async function run(opts = {}) {
  ...
  API = creds.api;
  PORT = creds.port;
  CAPTURE_SPEED = Number(captureEnv("SPEED", "8"));
  GAP = Number(captureEnv("GAP", "800"));
  HEADED = captureEnv("HEADED") === "1";
