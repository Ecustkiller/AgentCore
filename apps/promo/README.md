...
>
> **别搞混**：这支是「协作智能」主题的**脚本化复刻**品牌片；另有一支「谁更聪明」主题的**真录多模型辩论**视频（不同概念），见 [真·多模型辩论与谁更聪明视频](/docs/06-规划/真多模型辩论与谁更聪明视频.md)。

## 定档关键事实

① 30 秒**横屏**（16:9）、**仅横屏**；② 风格＝**真实产品演示**，与产品**像素一致**；③ 技术路线＝**纯 Remotion 复用真实组件**（「全 B」）；④ **亮色主题** + **桌面端外壳**；⑤ 字幕由本片产出、**BGM 用户后续自加**、无旁白；⑥ 片尾**无 CTA**（仅 Logo + slogan）；⑦ 字幕走「协作智能」品牌叙事；⑧ slogan＝「**AgentCore · 协作，是更高级的智能**」；⑨ demo＝4 层 DAG + 辩论（8 节点蝴蝶形）。
...
- **demo DAG**（8 Worker 节点，4 层，蝴蝶形）：L1 并行调研 ×3 →（交叉依赖）L2 辩论正反方（双向修订点线）→ L3 策略定稿 → L4 并行产出 ×2。一次展示并行 / 交叉依赖 / 辩论 / 波次调度 / 二次扇出全部核心编排能力。
...
| demo 任务 | **4 层 DAG + 辩论**：用户输入「做个完整的产品规划，正反两面论证后定方案」→ CEO 编排 8 节点蝴蝶形 DAG（3 并行调研 → 辩论正反方 → 裁决定稿 → 2 并行产出） |
...
- 另一支视频（不同概念）：[真·多模型辩论与谁更聪明视频](/docs/06-规划/真多模型辩论与谁更聪明视频.md)
