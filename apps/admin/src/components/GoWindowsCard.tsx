...
/**
 * OpenCode Go 5h / week / month — product CNY (DeepSeek official list) plus a
        ...
        <div className="flex items-start gap-2.5 rounded-xl border border-border bg-muted/40 px-4 py-3 text-sm text-muted-foreground">
          ...
          <span>
            {data
              ...
              : honestyCopy("deepseek-v4.1-flash", "2026-09-10")}
          </span>
        </div>
...
function honestyCopy(model: string, priceAsOf: string): string {
  return (
    `名义价按 DeepSeek 官方人民币列表价扣额度，不是 OpenCode 实付。` +
    `美元列按 Go 公开单价（${model}，截至 ${priceAsOf}）对每次调用按时段（Peak / Off-Peak）估算，` +
    `用来看离 $12 / $30 / $60 还有多远——不是上游账单或余额。` +
    `未证死：① Go 计入窗口前可能乘未公开的 costMultiplier，实际若大于 1 则美元列偏低；` +
